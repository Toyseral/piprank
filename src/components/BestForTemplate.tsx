...
export default function BestForTemplate({ document, brokers, ranked, intentSlug, criteria, faqs, countryName, countrySlug, localized, locale }: Props) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      ...
      {top && <section className="mt-10">
        <p className="text-xs font-bold uppercase tracking-widest text-emerald-700">Top match</p>
        <div className="mt-3 rounded-3xl border-2 border-emerald-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><p className="text-xs font-bold uppercase tracking-widest text-slate-400">PipRank recommendation</p><h2 className="mt-1 font-display text-2xl font-bold text-ink-950">{top.name}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{top.tagline}</p></div>
            <span className="rounded-full bg-ink-950 px-3 py-1.5 text-xs font-bold text-white">{top.rating}/100</span>
          </div>
          <div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-paper p-3"><p className="text-[10px] font-bold uppercase text-slate-400">EUR/USD spread</p><p className="mt-1 text-sm font-bold">{top.spread_eurusd} pips</p></div><div className="rounded-xl bg-paper p-3"><p className="text-[10px] font-bold uppercase text-slate-400">Minimum deposit</p><p className="mt-1 text-sm font-bold">{top.min_deposit}</p></div><div className="rounded-xl bg-paper p-3"><p className="text-[10px] font-bold uppercase text-slate-400">Regulation</p><p className="mt-1 text-sm font-bold">{top.regulations?.map((r) => r.body).filter(Boolean).join(', ') || 'See broker review'}</p></div></div>
          <div className="mt-5 flex flex-wrap gap-3"><ButtonLink variant="dark" size="md" to={`/brokers/${top.slug}`}>Read {top.name} Review</ButtonLink><ButtonLink variant="light" size="md" to={ctaPath}>Match Me With a Broker</ButtonLink></div>
        </div>
      </section>}
