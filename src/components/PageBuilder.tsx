...
function brokerCardHtml(b: Broker, variant: BrokerCardVariant = 'default') {
  ...
  const contextual = `<div class="min-w-0 rounded-xl bg-paper px-3 py-2"><p class="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Regulation</p><p class="tnum mt-0.5 truncate font-bold leading-tight text-ink-900 text-[12px] sm:text-sm">${esc(regulationLabel(b))}</p></div>`;
  const metrics = [
    `<div class="min-w-0 rounded-xl bg-paper px-3 py-2"><p class="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Min deposit</p><p class="tnum mt-0.5 truncate font-bold leading-tight text-ink-900 text-[12px] sm:text-sm">${esc(fmtMoney(b.min_deposit))}</p></div>`,
...
export function blocksToHtml(blocks: PageBlock[], brokers: Broker[] = []) {
  ...
  const slug = (s: string) => { const base = s.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'section'; const n = (seen.get(base) || 0) + 1; seen.set(base, n); return n > 1 ? `${base}-${n}` : base; };
  return blocks.map(b => {
    ...
    if (b.type === 'broker_grid') return `<div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">${(b.brokerIds || []).map(id => brokers.find(x => x.id === Number(id))).filter(Boolean).map(x => brokerCardHtml(x as Broker, (b.variant as BrokerCardVariant) || 'compact')).join('\n')}</div>`;
...
function BrokerBlockEditor({ block, brokers, onChange }: { block: PageBlock; brokers: Broker[]; onChange: (p: Partial<PageBlock>) => void }) { if (block.type === 'structured_broker_data') return <div className="space-y-3"><div><label className="label">Broker</label><BrokerPicker value={block.brokerId} brokers={brokers} onChange={brokerId => onChange({ brokerId})}/></div><div><label className="label">Data category</label><select value={block.section || 'overview'} onChange={e => onChange({ section: e.target.value as StructuredBrokerSection })} className="input">{(['overview','pricing','trust','platforms','features','editorial'] as StructuredBrokerSection[]).map(s => <option key={s} value={s}>{sectionLabel(s)}</option>)}</select></div><p className="text-[10px] text-slate-400">This block references the live Broker Workspace record. Updating the broker updates every block using this ID.</p></div>; if (block.type === 'broker_card') return <div className="space-y-3"><div><label className="label">Broker</label><BrokerPicker value={block.brokerId} brokers={brokers} onChange={brokerId => onChange({ brokerId})}/></div><div><label className="label">Card style</label><select value={block.variant || 'default'} onChange={e => onChange({ variant: e.target.value as BrokerCardVariant })} className="input"><option value="default">Default</option><option value="compact">Compact</option><option value="featured">Featured</option></select></div></div>; if (block.type === 'broker_grid') return <div className="space-y-3"><div><label className="label">Brokers</label><BrokerPicker value={block.brokerIds} brokers={brokers} multiple onChange={brokerIds => onChange({ brokerIds })}/></div><div><label className="label">Card style</label><select value={block.variant || 'compact'} onChange={e => onChange({ variant: e.target.value as BrokerCardVariant })} className="input"><option value="compact">Compact</option><option value="default">Default</option><option value="featured">Featured</option></select></div></div>; if (block.type === 'comparison_table') return <div className="space-y-3"><div><label className="label">Brokers</label><BrokerPicker value={block.brokerIds} brokers={brokers} multiple onChange={brokerIds => onChange({ brokerIds })}/></div><div><label className="label">Table title</label><input value={block.title || ''} onChange={e => onChange({ title: e.target.value })} className="input"/></div><div><label className="label">Comparison fields</label><div className="flex flex-wrap gap-2">{ALL_FIELDS.map(f => <label key={f} className="inline-flex items-center gap-1.5 rounded-lg border border-line bg-white px-2.5 py-1.5 text-[10px] font-bold"><input type="checkbox" checked={(block.fields || []).includes(f)} onChange={() => onChange({ fields: (block.fields || []).includes(f) ? (block.fields || []).filter(x => x !== f) : [...(block.fields || []), f] })}/>{fieldLabel(f)}</label>)}</div></div></div>; if (block.type === 'broker_cta') return <div className="space-y-3"><div><label className="label">Broker</label><BrokerPicker value={block.brokerId} brokers={brokers} onChange={brokerId => onChange({ brokerId})}/></div><div><label className="label">Headline</label><input value={block.headline ?? block.title ?? ''} onChange={e => onChange({ headline: e.target.value })} className="input"/></div><div><label className="label">Button label</label><input value={block.buttonLabel ?? block.ctaLabel ?? ''} onChange={e => onChange({ buttonLabel: e.target.value })} className="input"/></div><div><label className="label">Optional custom URL</label><input value={block.ctaHref || ''} onChange={e => onChange({ ctaHref: e.target.value })} className="input" placeholder="Leave blank to use broker affiliate URL"/></div><select value={block.variant || 'primary'} onChange={e => onChange({ variant: e.target.value as BrokerCtaVariant })} className="input"><option value="primary">Primary</option><option value="dark">Dark</option><option value="soft">Soft</option></select></div>; return null; }
function BrokerVisualPreview({ block, brokers }: { block: PageBlock; brokers: Broker[] }) { if (block.type === 'broker_card') { const broker = brokers.find(b => b.id === Number(block.brokerId)); if (!broker) return <div className="rounded-xl border border-dashed border-line p-5 text-xs text-slate-400">Select a broker to preview the live BrokerCard.</div>; return <div className="max-w-md"><BrokerCard broker={broker} /></div>; } if (block.type === 'broker_grid') { const selected = (block.brokerIds || []).map(id => brokers.find(b => b.id === Number(id))).filter(Boolean) as Broker[]; if (!selected.length) return <div className="rounded-xl border border-dashed border-line p-5 text-xs text-slate-400">Select brokers to preview the live broker cards.</div>; return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{selected.map((broker, i) => <BrokerCard key={broker.id} broker={broker} rank={i + 1} />)}</div>; } return null; }
function BlockEditor({ block, index, total, brokers, onChange, onMove, onDuplicate, onRemove, onUploadImage }: { block: PageBlock; index: number; total: number; brokers: Broker[]; onChange: (p: Partial<PageBlock>) => void; onMove: (i: number, d: number) => void; onDuplicate: (i: number) => void; onRemove: (i: number) => void; onUploadImage?: (file: File) => Promise<string> }) { return <div className="rounded-2xl border border-line bg-white shadow-sm"><div className="flex items-center gap-2 border-b border-line px-3 py-2"><span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{block.type.replaceAll('_',' ')}</span><span className="text-[10px] text-slate-300">#{index + 1}</span><span className="flex-1"/><button onClick={() => onDuplicate(index)} title="Duplicate" className="iconbtn"><Copy size={14}/></button><button disabled={index === 0} onClick={() => onMove(index, -1)} title="Move up" className="iconbtn"><ArrowUp size={14}/></button><button disabled={index === total - 1} onClick={() => onMove(index, 1)} title="Move down" className="iconbtn"><ArrowDown size={14}/></button><button onClick={() => onRemove(index)} title="Delete" className="iconbtn text-rose-500"><Trash2 size={14}/></button></div><div className="p-3">{block.type === 'heading' && <input value={block.title || ''} onChange={e => onChange({ title: e.target.value })} className="w-full rounded-xl border border-line px-3 py-2.5 font-display text-xl font-bold"/>}{block.type === 'richtext' && <RichTextEditor value={block.html || ''} onChange={html => onChange({ html })} onUploadImage={onUploadImage}/>} {block.type === 'image' && <div className="grid gap-3 sm:grid-cols-2"><div><label className="label">Image URL</label><input value={block.src || ''} onChange={e => onChange({ src: e.target.value })} className="input"/><label className="label mt-3">Alt text</label><input value={block.alt || ''} onChange={e => onChange({ alt: e.target.value })} className="input"/>{onUploadImage && <label className="mt-3 inline-flex cursor-pointer rounded-xl bg-ink-950 px-3 py-2 text-xs font-bold text-white">Upload image<input type="file" accept="image/*" className="hidden" onChange={async e => { const f = e.target.files?.[0]; if (!f) return; onChange({ src: await onUploadImage(f) }); e.currentTarget.value = ''; }}/></label>}</div><div className="flex min-h-40 items-center justify-center rounded-xl bg-slate-100">{block.src ? <img src={block.src} alt={block.alt || ''} className="max-h-64 rounded-xl object-contain"/> : <span className="text-xs text-slate-400">Image preview</span>}</div></div>}{block.type === 'table' && <TableBlock block={block} onChange={onChange}/>} {block.type === 'links' && <LinksBlock block={block} onChange={onChange}/>} {block.type === 'callout' && <div><select value={block.tone || 'neutral'} onChange={e => onChange({ tone: e.target.value as any })} className="mb-2 rounded-lg border border-line px-2 py-1 text-xs"><option>neutral</option><option>success</option><option>warning</option><option>dark</option></select><RichTextEditor value={block.html || ''} onChange={html => onChange({ html })}/></div>}{block.type === 'divider' && <hr/>}{['structured_broker_data','comparison_table','broker_cta'].includes(block.type) && <BrokerBlockEditor block={block} brokers={brokers} onChange={onChange}/>} {['broker_card','broker_grid'].includes(block.type) && <div className="space-y-4"><BrokerBlockEditor block={block} brokers={brokers} onChange={onChange}/><div className="rounded-xl border border-emerald-100 bg-emerald-50/30 p-3"><div className="mb-2 text-[10px] font-bold uppercase tracking-widest text-emerald-700">Live production card preview</div><BrokerVisualPreview block={block} brokers={brokers}/></div></div>}</div></div>; }
