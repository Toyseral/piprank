...
type Props = { value?: unknown[]; onChange: (blocks: PageBlock[]) => void; onUploadImage?: (file: File) => Promise<string>; context?: PageBuilderContext };
...
function getInsertTypes(context: PageBuilderContext = 'default') {
  if (context === 'broker-editorial') {
    return INSERT_TYPES.filter(
      (item) => item.type !== 'structured_broker_data'
    );
  }
  return INSERT_TYPES;
}
...
export default function PageBuilder({ value, onChange, onUploadImage, context = 'default' }: Props) {
  ...
  const emit = (next: PageBlock[]) => { const filtered = context === 'broker-editorial' ? next.filter(block => block.type !== 'structured_broker_data') : next; setBlocks(filtered); onChange(filtered); };
