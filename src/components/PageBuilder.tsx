...
function comparisonHtml(block: PageBlock, brokers: Broker[]) {
  ...
  const cta = block.ctaLabel ? `<div class="mt-5 flex flex-wrap items-center gap-3"><a href="${esc(block.ctaHref || '/compare')}" class="inline-flex items-center justify-center rounded-xl bg-ink-950 px-4 py-2.5 text-sm font-bold text-white">${esc(block.ctaLabel)}</a></div>` : '';
  return `<section class="piprank-comparison-table"><h3>${esc(block.title || 'Broker comparison')}</h3><div class="overflow-x-auto"><table><thead><tr><th>Broker</th>${selected.map(b => `<th>${esc(b.name)}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table></div>${cta}</section>`;
}
...
function ctaHtml(block: PageBlock, brokers: Broker[]) {
  ...
  const classes = variant === 'dark'
    ...
    : variant === 'soft'
      ? 'border-emerald-200 bg-emerald-50 text-ink-900'
      : 'border-emerald-500 bg-emerald-500 text-ink-950';
  return `<aside class="piprank-broker-cta rounded-2xl border p-5 sm:p-6 ${classes}"><div class="flex flex-wrap items-center justify-between gap-4"><div><strong class="block font-display text-lg">${esc(headline)}</strong><span class="mt-1 block text-sm opacity-75">Review ${esc(b.name)}'s current pricing, regulation and trading conditions.</span></div><a href="${esc(href)}" class="inline-flex items-center justify-center rounded-xl bg-white/95 px-4 py-2.5 text-sm font-bold text-ink-950 shadow-sm">${esc(label)}</a></div></aside>`;
}
...
function makeBlock(type: PageBlock['type'], brokers: Broker[]): PageBlock {
  ...
  if (type === 'comparison_table') return { id: uid(), type, brokerIds: brokers.slice(0, 3).map(x => x.id), fields: ['rating', 'trust_score', 'min_deposit', 'spread_eurusd'], title: 'Broker comparison', ctaLabel: '', ctaHref: '' };
...
function BrokerBlockEditor({ block, brokers, onChange }: { block: PageBlock; brokers: Broker[]; onChange: (p: Partial<PageBlock>) => void }) {
  ...
  if (block.type === 'comparison_table') return <div className="space-y-3"><div><label className="label">Brokers</label><BrokerPicker value={block.brokerIds} brokers={brokers} multiple onChange={brokerIds => onChange({ brokerIds })}/></div><div><label className="label">Table title</label><input value={block.title || ''} onChange={e => onChange({ title: e.target.value })} className="input"/></div><div><label className="label">Comparison fields</label><div className="flex flex-wrap gap-2">{ALL_FIELDS.map(f => <label key={f} className="inline-flex items-center gap-1.5 rounded-lg border border-line bg-white px-2.5 py-1.5 text-[10px] font-bold"><input type="checkbox" checked={(block.fields || []).includes(f)} onChange={() => onChange({ fields: (block.fields || []).includes(f) ? (block.fields || []).filter(x => x !== f) : [...(block.fields || []), f] })}/>{fieldLabel(f)}</label>)}</div></div><div className="grid gap-3 sm:grid-cols-2"><div><label className="label">Optional CTA label</label><input value={block.ctaLabel || ''} onChange={e => onChange({ ctaLabel: e.target.value })} className="input" placeholder="Compare all brokers"/></div><div><label className="label">Optional CTA URL</label><input value={block.ctaHref || ''} onChange={e => onChange({ ctaHref: e.target.value })} className="input" placeholder="/compare"/></div></div></div>;
