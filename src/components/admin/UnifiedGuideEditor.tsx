...
const GUIDE_IMAGES = ['/images/guides/basics.jpg','/images/guides/risk.jpg','/images/guides/psychology.jpg','/images/guides/platforms.jpg','/images/guides/costs.jpg','/images/guides/strategy.jpg'];
...
type Props = {
  ...
  defaultContentType?: 'guide' | 'country-guide' | 'localized-guide';
  defaultCountrySlug?: string;
  languageCode?: string;
  languagePrefix?: string;
};

export default function UnifiedGuideEditor({ document, countries: _countries, brokers: _brokers, token, onClose, onSave, defaultContentType = 'country-guide', defaultCountrySlug = '', languageCode = '', languagePrefix = '' }: Props) {
  const isNew = !document || document.id === 0;
  const initialSettings = document?.settings || {};
  const [form, setForm] = useState<any>(() => document ? {
    ...document,
    settings: initialSettings,
    category: initialSettings.category || 'Basics',
    level: initialSettings.level || 'Beginner',
    minutes: initialSettings.minutes ?? 8,
    image: initialSettings.image || GUIDE_IMAGES[0],
  } : {
  ...
  useEffect(() => { fetch('/api/content-documents?type=author').then((r) => r.ok ? r.json() : []).then((rows) => setAuthors(Array.isArray(rows) ? rows : [])).catch(() => setAuthors([])); }, []);
  const [preview, setPreview] = useState(false);
  const settings = form.settings || {};
  const [faqs, setFaqs] = useState<any[]>(settings.faqs || []);
  const [links, setLinks] = useState<any[]>(settings.internalLinks || []);

  const route = form.content_type === 'guide' && form.slug ? `/guides/${form.slug}` : form.content_type === 'localized-guide' && form.country_slug && form.slug && languagePrefix ? `/${form.country_slug}/${languagePrefix}/guides/${form.slug}` : form.content_type === 'country-guide' && form.country_slug && form.slug ? `/${form.country_slug}/guides/${form.slug}` : '#';
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!['guide', 'country-guide', 'localized-guide'].includes(type)) throw new Error('Only guide content types can be saved here.');
      const key = form.content_key || (type === 'localized-guide' ? `localized-guide:${form.country_slug}:${languageCode || settings.languageCode || ''}:${slug}` : type === 'country-guide' ? `country-guide:${form.country_slug}:${slug}` : `guide:${slug}`);
      await onSave({ ...form, content_type: type, slug, content_key: key, html: blocksToHtml(form.blocks || []), settings: { ...settings, category: form.category, level: form.level, minutes: Number(form.minutes) || 0, image: form.image, faqs, internalLinks: links, author_slug: String(settings.author_slug || '').trim().toLowerCase(), reviewed_by_slug: String(settings.reviewed_by_slug || '').trim().toLowerCase(), ...(languageCode ? { languageCode } : {}) }, ...(isNew ? {} : { id: document!.id }) }, isNew);
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-ink-950/60 p-2 backdrop-blur-sm">
      <div className="flex h-full flex-col overflow-hidden rounded-3xl bg-paper shadow-soft-lg">
        <header className="flex items-center gap-3 bg-ink-950 px-5 py-4 text-white">
          <FileText size={20} className="text-emerald-400" />
          <div className="min-w-0 flex-1"><p className="font-display text-lg font-bold">{isNew ? 'Create guide' : 'Edit guide'}</p><p className="truncate text-xs text-slate-400">{route} · {form.content_type === 'localized-guide' ? 'localized guide' : form.content_type === 'country-guide' ? 'country guide' : 'global guide'}</p></div>
          {route !== '#' && <a href={`${route}${form.content_type === 'localized-guide' ? '?preview=1' : ''}`} target="_blank" rel="noreferrer" className="hidden rounded-xl border border-white/10 px-3 py-2 text-xs font-bold sm:inline-flex"><Eye size={14} className="mr-1.5" /> Preview</a>}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6"><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]"><main className="space-y-5">
          <section className="rounded-2xl border border-line bg-white p-5"><div className="grid gap-3 sm:grid-cols-2">
            ...
            {!fixedContext && <label><b className="text-xs">Country</b><select value={form.country_slug || ''} onChange={e => setForm({ ...form, country_slug: e.target.value })} className={input}><option value="">Select country</option>{_countries.map((c: CountryPage) => <option key={c.slug} value={c.slug}>{c.name}</option>)}</select></label>}
        ...
        </main><aside className="space-y-4">
          <section className="rounded-2xl border border-line bg-white p-4"><b className="font-display">Attribution</b><label className="mt-3 block"><span className="text-xs font-bold">Written / reviewed by</span><select value={String(settings.reviewed_by_slug || settings.author_slug || '')} onChange={e => setForm({ ...form, settings: { ...(form.settings || {}), author_slug: e.target.value, reviewed_by_slug: e.target.value } })} className={`${input} mt-1.5`}><option value="">Use deterministic fallback</option>{authors.map(a => <option key={a.slug || a.id} value={a.slug || ''}>{a.title}</option>)}</select></label></section>
