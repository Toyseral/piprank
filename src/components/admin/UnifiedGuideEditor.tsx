...
export default function UnifiedGuideEditor({ document, countries, brokers, token, onClose, onSave, defaultContentType = 'country-guide', defaultCountrySlug = '', languageCode = '', languagePrefix = '' }: Props) {
  ...
  const save = async () => {
    ...
    setErr('');
    try {
      if (isTopic && rankingMode === 'manual' && pinned.length === 0) {
        throw new Error('Manual ranking requires at least one selected eligible broker.');
      }
      ...
      const key = form.content_key || (
        type === 'localized-guide' ? `localized-guide:${form.country_slug}:${languageCode || settings.languageCode || ''}:${slug}` :
        type === 'country-guide' ? `country-guide:${form.country_slug}:${slug}` :
        type === 'guide' ? `guide:${slug}` :
        `country-topic:${form.country_slug}:${form.topic_slug}`
      );
      ...
      await onSave({
        ...
        settings: {
          ...
          image: form.image,
          rankingMode,
          pinnedBrokerSlugs: cleanPinned,
          excludedBrokerSlugs: excluded,
          faqs,
          internalLinks: links,
          ...(languageCode ? { languageCode } : {}),
        },
        ...(isNew ? {} : { id: document!.id }),
      }, isNew);
    } catch (e) {
      ...
      <div className="flex h-full flex-col overflow-hidden rounded-3xl bg-paper shadow-soft-lg">
        <header className="flex items-center gap-3 bg-ink-950 px-5 py-4 text-white">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-display text-lg font-bold">{isNew ? 'Create guide' : 'Edit guide'}</p>
            <p className="truncate text-xs text-slate-400">{route} · {form.content_type === 'localized-guide' ? 'localized guide' : form.content_type === 'country-guide' ? 'country guide' : form.content_type === 'guide' ? 'global guide' : 'SEO content'}</p>
          </div>
          {route !== '#' && (
            <a href={`${route}${form.content_type === 'localized-guide' ? '?preview=1' : ''}`} target="_blank" rel="noreferrer" className="hidden rounded-xl border border-white/10 px-3 py-2 text-xs font-bold sm:inline-flex">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
            <main className="space-y-5">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {fixedContext ? (
                    <div>
                      ...
                      <p className="mt-1 font-display text-sm font-bold text-ink-900">{form.content_type === 'localized-guide' ? `Localized guide · ${form.country_slug} · ${languageCode || settings.languageCode || 'language'}` : form.content_type === 'country-guide' ? `Country guide · ${form.country_slug || 'country not selected'}` : 'Global guide'}</p>
                    </div>
                  ) : (
                    <label><b className="text-xs">Page type</b><select value={form.content_type} onChange={e => setForm({ ...form, content_type: e.target.value })} className={input}><option value="country-topic">Country SEO</option><option value="country-guide">Country guide</option><option value="localized-guide">Localized guide</option><option value="guide">Global guide</option></select></label>
                  )}
              ...
              <section className="rounded-2xl border border-line bg-white p-5">
                <div className="flex items-center justify-between"><div><b className="font-display">Article content</b><p className="text-xs text-slate-400">Build the article with the same reorderable content blocks used by the global guide editor.</p></div><button onClick={() => setPreview(v => !v)} className="rounded-xl border border-line px-3 py-2 text-xs font-bold">{preview ? 'Edit' : 'Preview'}</button></div>
                {preview ? <div className="piprank-rich-content mt-5 rounded-2xl border border-line bg-paper p-5" dangerouslySetInnerHTML={{ __html: blocksToHtml(form.blocks || []) }} /> : <div className="mt-3"><PageBuilder value={form.blocks || []} onChange={blocks => setForm({ ...form, blocks, html: blocksToHtml(blocks) })} onUploadImage={uploadImage} /></div>}
              </section>
            </main>
            ...
            <aside className="space-y-4">
              ...
              <button onClick={save} disabled={busy} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-ink-950 text-sm font-bold text-white disabled:opacity-60"><Save size={15} />{busy ? <><Loader2 size={14} className="animate-spin" /> Saving…</> : form.published ? 'Save & publish' : 'Save draft'}</button>
