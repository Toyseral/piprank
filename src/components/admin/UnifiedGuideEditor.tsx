...
export default function UnifiedGuideEditor({ document, countries: _countries, brokers: _brokers, token, onClose, onSave, defaultContentType = 'country-guide', defaultCountrySlug = '', languageCode = '', languagePrefix = '' }: Props) {
  ...
  const save = async () => {
    ...
    try {
      ...
      await onSave({ ...form, content_type: type, slug, content_key: key, html: blocksToHtml(form.blocks || []), settings: { ...settings, category: form.category, level: form.level, minutes: Number(form.minutes) || 0, image: form.image, faqs, internalLinks: links, ...(languageCode ? { languageCode } : {}) }, ...(isNew ? {} : { id: document!.id }) }, isNew);
