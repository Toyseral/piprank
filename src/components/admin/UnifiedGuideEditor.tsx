...
export default function UnifiedGuideEditor({ document, countries, brokers, token, onClose, onSave, defaultContentType = 'country-guide', defaultCountrySlug = '', languageCode = '', languagePrefix = '' }: Props) {
  ...
  const manualPool = useMemo(() => {
    ...
    const country = countries.find((c: CountryPage) => c.slug === form.country_slug);
  ...
  const unavailablePinned = useMemo(() => pinned.filter((slug: string) => !manualPool.some((b: Broker) => b.slug === slug)), [pinned, manualPool]);
  ...
  const save = async () => {
    ...
    try {
      ...
      await onSave({ ...form, content_type: type, slug, content_key: key, html: blocksToHtml(form.blocks || []), settings: { ...settings, category: form.category, level: form.level, minutes: Number(form.minutes) || 0, image: form.image, rankingMode, pinnedBrokerSlugs: Array.from(new Set(pinned)), excludedBrokerSlugs: excluded, faqs, internalLinks: links, ...(languageCode ? { languageCode } : {}) }, ...(isNew ? {} : { id: document!.id }) }, isNew);
    ...
    <div className="fixed inset-0 z-[120] bg-ink-950/60 p-2 backdrop-blur-sm">
      <div className="flex h-full flex-col overflow-hidden rounded-3xl bg-paper shadow-soft-lg">
        <header className="flex items-center gap-3 bg-ink-950 px-5 py-4 text-white">
          ...
          <div className="min-w-0 flex-1"><p className="font-display text-lg font-bold">{isNew ? 'Create guide' : 'Edit guide'}</p><p className="truncate text-xs text-slate-400">{route} · {form.content_type === 'localized-guide' ? 'localized guide' : form.content_type === 'country-guide' ? 'country guide' : form.content_type === 'guide' ? 'global guide' : 'SEO content'}</p></div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6"><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]"><main className="space-y-5">
          <section className="rounded-2xl border border-line bg-white p-5"><div className="grid gap-3 sm:grid-cols-2">
            ...
            {!fixedContext && <label><b className="text-xs">Country</b><select value={form.country_slug || ''} onChange={e => setForm({ ...form, country_slug: e.target.value })} className={input}><option value="">Select country</option>{countries.map((c: CountryPage) => <option key={c.slug} value={c.slug}>{c.name}</option>)}</select></label>}
