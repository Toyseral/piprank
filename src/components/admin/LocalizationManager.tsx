...
export function LocalizationManager({ countries, languages, contentDocs, mutate }: {
  ...
  const addLanguage = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      await mutate('/api/canonical-country-languages', 'POST', { country_id: countryId, name, native_name: nativeName, code, locale, url_prefix: prefix || code }, `${nativeName} localization created`);
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      {countryLanguages.map((language) => {
        ...
        return <div key={language.id} className={`rounded-2xl border bg-white p-5 ${open ? 'border-emerald-300' : 'border-line'}`}>
          <div className="flex items-center gap-3"><div className="min-w-0 flex-1"><p className="font-bold text-ink-950">{language.native_name} <span className="font-normal text-slate-400">({language.name})</span></p><p className="mt-1 text-xs text-slate-500">{language.country_name} · {language.locale} · /{language.url_prefix}/ · {language.active ? 'Active' : 'Inactive'}</p><p className="mt-1 text-xs font-semibold text-emerald-700">{languageBestFors.length} localized Best-For · {languageDocs.length} localized documents</p></div><button onClick={() => { setCountryId(language.country_id); setSelectedLanguage(open ? null : language.id); }} className="rounded-xl border border-line px-3 py-2 text-xs font-bold">{open ? 'Hide pages' : 'Manage pages'}</button><button onClick={() => mutate('/api/canonical-country-languages', 'PUT', { id: language.id, active: !language.active }, language.active ? 'Language disabled' : 'Language enabled')} className="rounded-xl border border-line px-3 py-2 text-xs font-bold">{language.active ? 'Disable' : 'Enable'}</button></div>
