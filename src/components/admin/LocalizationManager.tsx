...
export function LocalizationManager({ countries, languages, contentDocs, mutate }: {
  ...
  const docs = useMemo(
    () => contentDocs.filter((doc) => CANONICAL_TYPES.has(doc.content_type)),
    [contentDocs]
  );
  ...
  const selectedDocs = useMemo(() => {
    ...
    return docs
      .filter((doc) => doc.country_slug === country.slug && localeFromKey(doc.content_key) === selectedLang.code)
  ...
  const localizedGuides = selectedDocs.filter((doc) => doc.content_type === 'localized-guide');
  ...
  const addLanguage = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      await mutate('/api/country-languages', 'POST', {
  ...
  const addTopic = async (topicKey: string) => {
    ...
    await mutate('/api/content-documents', 'POST', {
      ...
      html: (template.intro ?? [])
        .map((p) => `<p>${String(p).replace(/[&<>]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c] || c))}</p>`)
        .join(''),
      blocks: [],
      settings: {
  ...
  const addableTopics = countrySeoTopics.filter(
    (topic) => !localizedBestFors.some((doc) => topicFromDoc(doc) === topic.key)
  );
  ...
  return <div className="space-y-6">
    <div className="rounded-3xl border border-line bg-white p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 font-display text-xl font-bold text-ink-950">Localization Studio</h2>
          <p className="mt-2 text-sm leading-6 text-slate-500">Localized guides and Best-For pages are canonical Content Studio documents. Best-For pages are created and edited here; legacy localized-seo pages are not used.</p>
        </div>
        <Globe2 className="text-emerald-600" size={22} />
      </div>
      <form onSubmit={addLanguage} className="mt-6 grid gap-3 md:grid-cols-3">
        <select value={countryId} onChange={(e) => setCountryId(Number(e.target.value))} className="h-11 rounded-xl border border-line bg-paper px-3 text-sm">
        ...
        <button disabled={saving} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-ink-950 px-4 text-sm font-bold text-white disabled:opacity-60 md:col-span-3">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      {countryLanguages.map((language) => {
        const languageDocs = docs.filter((doc) => doc.country_slug === language.country_name && localeFromKey(doc.content_key) === language.code);
        const languageBestFors = docs.filter((doc) => doc.content_type === 'localized-best-for' && localeFromKey(doc.content_key) === language.code && doc.country_slug === countries.find((c) => c.id === language.country_id)?.slug);
        ...
        return <div key={language.id} className={`rounded-2xl border bg-white p-5 ${open ? 'border-emerald-300' : 'border-line'}`}>
          <div className="flex items-center gap-3">
            <div className="min-w-0 flex-1">
              <p className="font-bold text-ink-950">{language.native_name} <span className="font-normal text-slate-400">({language.name})</span></p>
              <p className="mt-1 text-xs text-slate-500">{language.country_name} · {language.locale} · /{language.url_prefix}/ · {language.active ? 'Active' : 'Inactive'}</p>
              <p className="mt-1 text-xs font-semibold text-emerald-700">{languageBestFors.length} localized Best-For · {languageDocs.length} localized documents</p>
            </div>
          ...
          {open && <div className="mt-5 space-y-4 border-t border-line pt-4">
            <div className="rounded-xl border border-emerald-100 bg-emerald-50/50 p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div><p className="text-sm font-bold text-ink-950">Localized Best-For pages</p><p className="text-xs text-slate-500">{localizedBestFors.length} existing pages for {language.native_name}.</p></div>
                <span className="rounded-full bg-white px-2.5 py-1 text-xs font-bold text-emerald-700">Canonical content_documents</span>
              </div>
            </div>
...
function LocalizedDocumentRow({ doc, mutate }: { doc: ContentDocument; mutate: Mutate }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await mutate('/api/content-documents', 'PUT', {
        id: doc.id,
        title,
  ...
  return <div className="rounded-xl border border-line bg-paper p-4">
    <div className="flex items-center gap-3">
      <div className="min-w-0 flex-1">
        <p className="font-semibold text-ink-950">{doc.title || doc.slug}</p>
        <p className="text-xs text-slate-500">{doc.slug} · /{doc.country_slug}/{String(doc.settings?.locale || doc.settings?.languageCode || '')}/{doc.slug} · {doc.published ? 'Published' : 'Draft'}{doc.indexable ? ' · Indexable' : ' · Noindex'}</p>
      </div>
      <button type="button" onClick={() => setExpanded(!expanded)} className="rounded-lg border border-line bg-white px-3 py-2 text-xs font-bold">{expanded ? 'Close' : 'Edit'}</button>
    </div>
    {expanded && <div className="mt-4 space-y-3">
      <input value={title} onChange={(e) => setTitle(e.target.value)} className="h-10 w-full rounded-lg border border-line bg-white px-3 text-sm" placeholder="Title" />
      <input value={seoTitle} onChange={(e) => setSeoTitle(e.target.value)} className="h-10 w-full rounded-lg border border-line bg-white px-3 text-sm" placeholder="SEO title" />
      <textarea value={seoDescription} onChange={(e) => setSeoDescription(e.target.value)} className="min-h-20 w-full rounded-lg border border-line bg-white p-3 text-sm" placeholder="SEO description" />
      ...
      <div className="flex flex-wrap items-center gap-4 text-sm">
        ...
        <label className="flex items-center gap-2"><input type="checkbox" checked={indexable} onChange={(e) => setIndexable(e.target.checked)} /> Indexable</label>
      </div>
      <div className="flex gap-2">
        <button type="button" disabled={saving} onClick={save} className="inline-flex items-center gap-2 rounded-lg bg-ink-950 px-4 py-2 text-xs font-bold text-white"><Save size={14} /> {saving ? 'Saving…' : 'Save'}</button>
        <button type="button" onClick={remove} className="inline-flex items-center gap-2 rounded-lg border border-rose-200 px-4 py-2 text-xs font-bold text-rose-700"><Trash2 size={14} /> Delete</button>
      </div>
    </div>}
  </div>;
}
