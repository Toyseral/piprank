...
export default function CanonicalEditorLauncher({ token, brokers, countries: _countries, kind, countrySlug, topicSlug, onClose, onSaved }: { token:string; brokers:Broker[]; countries:CountryPage[]; kind:'global'|'country'; countrySlug?:string; topicSlug:string; onClose:()=>void; onSaved:()=>Promise<void>|void }) {
 const [doc,setDoc]=useState<ContentDocument|null>(null);
 const [loading,setLoading]=useState(true);
 const canonicalTopic=CANONICAL[topicSlug]||topicSlug;
 const key=kind==='global' ? `best-for:${canonicalTopic}` : `country-topic:${countrySlug}:${canonicalTopic}`;
 useEffect(()=>{let active=true;setLoading(true);fetch(`/api/content-documents?key=${encodeURIComponent(key)}`).then(r=>r.ok?r.json():null).then(d=>{if(active)setDoc(d)}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[key]);
