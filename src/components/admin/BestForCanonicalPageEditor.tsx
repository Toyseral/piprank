...
export default function BestForCanonicalPageEditor({ kind, document, brokers, countries, token, countrySlug, topicSlug, locale, onClose, onSave }: Props) {
  ...
  const analysisBrokers = useMemo(() => {
    const bySlug = new Map(manualPool.map((broker) => [broker.slug, broker]));
    const ordered = rankingMode === 'manual' ? pinned.map((slug) => bySlug.get(slug)).filter(Boolean) as Broker[] : manualPool;
    return [...ordered, ...manualPool.filter((broker) => !ordered.some((item) => item.slug === broker.slug))].slice(0, 9);
  }, [manualPool, pinned, rankingMode]);
  ...
  const save = async () => { try { setBusy(true); setError(''); if (rankingMode === 'manual' && pinned.length === 0) throw new Error('Manual ranking requires at least one selected broker.'); const settings = (form.settings ?? {}) as Record<string, any>; const payload = { ...form, settings: { ...settings, ...(kind === 'localized' ? { locale: String(settings.locale || locale || '').trim().slice(0, 40) } : {}), rankingMode, pinnedBrokerSlugs: Array.from(new Set(pinned)), excludedBrokerSlugs: Array.from(new Set(excluded)) }, blocks, html: blocksToHtml(blocks, brokers) } as ContentDocument; await onSave(payload, !document); } catch (e) { setError(e instanceof Error ? e.message : 'Could not save Best-For page'); } finally { setBusy(false); } };
  ...
  return <div className="fixed inset-0 z-[130] flex items-center justify-center bg-ink-950/60 p-3 backdrop-blur-sm"><div className="flex max-h-[96vh] w-full max-w-7xl flex-col overflow-hidden rounded-3xl bg-white shadow-soft-lg">
    ...
    <main className="flex-1 overflow-y-auto p-5 sm:p-7">
      ...
      <section className="rounded-2xl border border-line bg-paper p-4 sm:p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-emerald-700">Hero</p><h3 className="mt-1 font-display text-lg font-bold text-ink-950">Structured hero content</h3><p className="mt-1 text-xs text-slate-500">The hero is template-controlled; edit the page identity and supporting fields here.</p></div></div><div className="mt-4 grid gap-4 sm:grid-cols-2"><label><span className="text-xs font-bold">Title / H1</span><input value={form.title || ''} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className="mt-1.5 w-full rounded-xl border border-line bg-white px-3 py-2.5 text-sm" /></label><label><span className="text-xs font-bold">Hero eyebrow</span><input value={String(settings.heroEyebrow || '')} onChange={e => setForm(f => ({ ...f, settings: { ...(f.settings || {}), heroEyebrow: e.target.value } }))} placeholder="PipRank Best For" className="mt-1.5 w-full rounded-xl border border-line bg-white px-3 py-2.5 text-sm" /></label></div><label className="mt-4 block"><span className="text-xs font-bold">Hero supporting text</span><textarea value={form.excerpt || ''} onChange={e => setForm(f => ({ ...f, excerpt: e.target.value }))} rows={3} className="mt-1.5 w-full rounded-xl border border-line bg-white px-3 py-2.5 text-sm" /></label><div className="mt-4 max-w-md"><label><span className="text-xs font-bold">Hero CTA label</span><input value={String(settings.heroCta || 'Match Me With a Broker')} onChange={e => setForm(f => ({ ...f, settings: { ...(f.settings || {}), heroCta: e.target.value } }))} className="mt-1.5 w-full rounded-xl border border-line bg-white px-3 py-2.5 text-sm" /></label></div></section>
