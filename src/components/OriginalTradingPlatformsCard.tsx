    ...
    <section className="scroll-mt-28 rounded-3xl border border-line bg-white p-6 sm:p-8">
      ...
      <div className="mt-5 flex gap-1 overflow-x-auto border-b border-line scrollbar-none">
        {platformContent.map((p, i) => (
          <button key={p.name} onClick={() => setPlatformTab(i)} className={`whitespace-nowrap border-b-2 px-4 py-2.5 text-sm font-bold transition ${i === platformTab ? 'border-emerald-600 text-ink-900' : 'border-transparent text-slate-400 hover:text-ink-900'}`}>
            {p.name}
          </button>
        ))}
      </div>
      {activePlatform && (
        <div className="mt-5 rounded-2xl border border-line bg-paper p-5 sm:p-6">
          ...
          {activePlatform.features.length > 0 && <ul className="mt-4 grid gap-2 sm:grid-cols-3">{activePlatform.features.map((f) => <li key={f} className="flex items-start gap-2 rounded-xl bg-white px-3.5 py-2.5 text-xs font-semibold leading-snug text-slate-700 ring-1 ring-line"><Check size={13} className="mt-0.5 shrink-0 text-emerald-600" strokeWidth={3} />{f}</li>)}</ul>}
        </div>
      )}
      <div className="mt-6 grid grid-cols-3 gap-3">
        {[{ l: 'Median execution', v: `${broker.execution_ms} ms` }, { l: 'Platform uptime (90d)', v: `${broker.uptime}%` }, { l: 'Tradable symbols', v: Object.values(broker.assets).reduce((a, b) => a + b, 0).toLocaleString() }].map((s) => <div key={s.l} className="rounded-2xl bg-paper p-4 text-center"><p className="tnum font-display text-xl font-bold text-ink-900 sm:text-2xl">{s.v}</p><p className="mt-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{s.l}</p></div>)}
      </div>
