...
export const fetchIntent = async (slug: string) => {
  ...
  catch (firstError) { if (mapped === slug) throw firstError; return get<Intent>(`/api/intents?slug=${encodeURIComponent(slug)}`); }
};
...
export const fetchContentDocument = async (key: string): Promise<ContentDocument | null> => {
  ...
  if (parts.length === 3 && (parts[0] === 'country-topic' || parts[0] === 'country-guide')) {
    ...
    } catch { /* Preserve normal not-found behavior when fallback data is unavailable. */ }
  }
  return null;
};
