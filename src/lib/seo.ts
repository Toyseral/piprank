...
export function bestForPath(slug: string): string {
...
export function buildWebPageJsonLd(input: SeoInput) {
  return {
    '@context': 'https://schema.org', '@type': input.type === 'article' ? 'Article' : 'WebPage',
    name: input.title, description: input.description, url: absoluteUrl(input.path),
...
interface BrokerLike {
  name: string; slug: string; tagline?: string | null; rating?: number | null;
  min_deposit?: number | null; spread_eurusd?: number | null; max_leverage?: string | null;
}
...
export function intentSeo(i: { title: string; slug: string; meta_title?: string | null; meta_description?: string | null; intro?: string[]; indexable?: boolean }): SeoInput {
  ...
  return {
    ...
    path: bestForPath(i.slug), type: 'website', noindex: i.indexable === false,
  };
}
...
export function countryBestForSeo(countrySlug: string, p: { title: string; slug: string; meta_title?: string | null; meta_description?: string | null; intro?: string[]; indexable?: boolean }): SeoInput {
  ...
  return {
    ...
    path: `/${countrySlug}/${BEST_FOR_CANONICAL[p.slug] ?? p.slug}`, type: 'website', noindex: p.indexable === false,
  };
}
...
export function brokerSeo(b: BrokerLike): SeoInput {
  ...
  return {
    ...
    description: b.tagline && b.tagline !== 'New broker under review'
    ...
    path: `/brokers/${b.slug}`, type: 'article',
  };
}
...
export function countrySeo(c: CountryLike, pathOverride?: string): SeoInput {
  ...
  return {
    ...
    description: c.seo_description?.trim() || (c.subtitle?.trim() ? `${c.subtitle} Compare regulated forex brokers available to traders in ${c.name}, with real-money tested spreads and fees.` : `Compare the best forex brokers available to traders in ${c.name}. Regulation, spreads, deposit methods and verdicts — independently tested by ${SITE_NAME}.`),
    path: pathOverride || `/countries/${c.slug}`, type: 'website',
  };
}
...
export function guideSeo(g: GuideLike): SeoInput {
  return { title: `${g.title} | ${SITE_NAME} Guides`, description: g.excerpt?.trim() ? g.excerpt : `${g.title} — a ${g.category ?? 'trading'} guide from ${SITE_NAME}.`, path: `/guides/${g.slug}`, type: 'article' };
}

export function comparePairSeo(a: BrokerLike, b: BrokerLike): SeoInput {
  const year = new Date().getFullYear();
  return { title: `${a.name} vs ${b.name} (${year}): Which Is Better? | ${SITE_NAME}`, description: `${a.name} vs ${b.name} head-to-head: spreads, fees, leverage, regulation and execution speed compared side by side, with a clear verdict.`, path: `/compare/${a.slug}-vs-${b.slug}`, type: 'article' };
}

export const staticPageSeo = {
  brokers: { title: `Best Forex Brokers ${new Date().getFullYear()}: Full Rankings | ${SITE_NAME}`, description: 'Every broker in our directory, ranked by real-money tested spreads, execution speed, regulation and fees. Filter and compare in seconds.', path: '/brokers', type: 'website' as const },
  countries: { title: `Best Forex Brokers by Country | ${SITE_NAME}`, description: 'Find the best regulated forex brokers available in your country, with local payment methods, tax notes and verified availability.', path: '/countries', type: 'website' as const },
  guides: { title: `Forex Trading Guides & Education | ${SITE_NAME}`, description: 'Learn how spreads, leverage, execution and regulation actually work, from beginner basics to advanced strategy — written by traders who test brokers for a living.', path: '/guides', type: 'website' as const },
  compare: { title: `Compare Forex Brokers Side by Side | ${SITE_NAME}`, description: 'Pick any two brokers and see a full side-by-side breakdown of spreads, fees, leverage, execution speed and regulation.', path: '/compare', type: 'website' as const },
  about: { title: `About PipRank: Editorial Policy, Methodology & Affiliate Disclosure | ${SITE_NAME}`, description: 'How PipRank works: our editorial independence policy, how we make money, broker review and scoring methodology, data sources, update cadence, corrections policy and reviewer information.', path: '/about', type: 'website' as const },
};
