export function buildCountryHubModel(input) {
  ...
  const countryDocument = input.countryDocument ?? {
    ...
    indexable: true, published: true, updated_by: null,
    created_at: new Date(0).toISOString(), updated_at: new Date(0).toISOString(), settings: {},
  };
  ...
  const eligibleBrokerIds = new Set(input.brokers.filter((broker) => !ineligibleIds.has(Number(broker.id))).map((broker) => Number(broker.id)));
  const faqs = Array.isArray(countryDocument.settings?.faqs)
    ? countryDocument.settings.faqs.filter((faq) => Boolean(faq && typeof faq.q === 'string' && typeof faq.a === 'string'))
    : [];
  return {
    country, countryDocument,
    availableBrokers: input.brokers.filter((broker) => eligibleBrokerIds.has(Number(broker.id))),
    topBrokers: input.topBrokers.filter((row) => row.broker && eligibleBrokerIds.has(Number(row.broker_id)) && row.availability_status === 'available'),
    ...
    faqs, comparisonPath: '/compare', methodologyPath: '/methodology',
  };
}
