...
function localeOf(document: Pick<ContentDocument, 'settings'>): string { const settings = document.settings ?? {}; return String((typeof settings.locale === 'string' ? settings.locale : '') || (typeof settings.languageCode === 'string' ? settings.languageCode : '')).trim(); }
...
export function canonicalPathForDocument(document: Pick<ContentDocument, 'content_type' | 'country_slug' | 'topic_slug' | 'slug' | 'settings'>): string | null {
  const country = document.country_slug ? encode(document.country_slug) : null; const slug = document.slug ? encode(document.slug) : null; const locale = localeOf(document);
...
export function canonicalContentKeyForDocument(document: Pick<ContentDocument, 'content_type' | 'country_slug' | 'topic_slug' | 'slug' | 'settings'>): string | null {
  const country = document.country_slug || ''; const slug = document.slug || ''; const locale = localeOf(document);
  switch (document.content_type) {
...
export async function resolveCanonicalPath(pathname: string): Promise<CanonicalRoute | null> {
  ...
  if (segments.length === 4 && segments[2] === 'guides') {
    const [countrySlug, locale, , slug] = segments; const country = await fetchCountry(countrySlug).catch(() => null); const document = await fetchPublicContentDocumentByKey(`localized-guide:${countrySlug}:${locale}:${slug}`);
  ...
  if (segments.length === 3) {
    const [countrySlug, locale, slug] = segments; const country = await fetchCountry(countrySlug).catch(() => null); const document = await fetchPublicContentDocumentByKey(`localized-best-for:${countrySlug}:${locale}:${slug}`);
  ...
  if (segments.length === 2) {
    const [countrySlug, slug] = segments; const [country, countryBestFor] = await Promise.all([fetchCountry(countrySlug).catch(() => null), fetchPublicContentDocumentByKey(`country-best-for:${countrySlug}:${slug}`)]);
  ...
  if (segments.length === 1) {
    const slug = segments[0]; const bestForDocument = await fetchPublicContentDocumentByKey(`best-for:${slug}`);
