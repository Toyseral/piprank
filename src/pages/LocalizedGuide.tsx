...
import { fetchLocalizedGuide, fetchCountry, fetchCountryLanguages } from '../lib/api';
...
export default function LocalizedGuide() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchCountry(countrySlug), fetchCountryLanguages(countrySlug)])
      .then(async ([c, langs]) => {
        ...
        setCountry(c);
        setLanguage(lang);
        if (!c || !lang) { setMissing(true); return; }
        const d = await fetchLocalizedGuide(c.slug, lang.code, slug);
        if (!alive) return;
  ...
  return <main className="mx-auto max-w-4xl px-5 py-10 sm:px-8 sm:py-16"><nav className="mb-8 text-xs text-slate-400">{country?.name} · {language?.native_name} · Guides</nav><article><h1 className="font-display text-4xl font-bold tracking-tight text-ink-950 sm:text-5xl">{doc.title}</h1>{doc.excerpt && <p className="mt-5 text-lg leading-8 text-slate-600">{doc.excerpt}</p>}<div className="prose prose-slate mt-10 max-w-none" dangerouslySetInnerHTML={{ __html: html }} /></article></main>;
}
