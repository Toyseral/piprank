...
const MANAGE_ROLES = new Set(['super_admin', 'admin', 'content_admin']);
...
export default function RankingWorkspace({ countries, intents, brokers, token }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [rankingResponse, modeResponse] = await Promise.all([
        ...
        supabase.rpc('get_country_intent_ranking_mode', {
      ...
      if (!modeResponse.error && (modeResponse.data === 'automatic' || modeResponse.data === 'manual')) {
  ...
  const saveManualRank = async (row: CountryIntentBrokerRanking, nextRank: number) => {
    if (mode !== 'manual' || !selectedCountry || !selectedIntent) return;
    ...
    if (!response.ok) {
      ...
      setMessage(data.error || 'Could not save manual rank.');
      setSavingRank(null);
      return;
    }
  ...
  const move = async (index: number, direction: -1 | 1) => {
    ...
    await Promise.all([
      saveManualRank(current, target.manual_rank ?? target.final_rank),
      saveManualRank(target, current.manual_rank ?? current.final_rank),
    ]);
  };

  const resetToAutomatic = () => {
    void setRankingMode('automatic');
  };
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
              Choose a country and intent, then decide whether the public broker order is calculated automatically or controlled manually for that exact pair.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Automatic uses the scoring engine. Manual uses the broker order below. Other country + intent pairs are unaffected.
              </p>
            </div>
            <div className="inline-flex rounded-xl border border-line bg-white p-1">
              <button type="button" onClick={() => void setRankingMode('automatic')} disabled={savingMode} className={`rounded-lg px-4 py-2 text-xs font-bold ${mode === 'automatic' ? 'bg-ink-950 text-white' : 'text-slate-500 hover:bg-paper'}`}>
                Automatic
              </button>
              <button type="button" onClick={() => void setRankingMode('manual')} disabled={savingMode} className={`rounded-lg px-4 py-2 text-xs font-bold ${mode === 'manual' ? 'bg-emerald-600 text-white' : 'text-slate-500 hover:bg-paper'}`}>
                Manual
              </button>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-3 border-b border-line p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              {mode === 'manual' ? 'Manual order is active for this pair.' : 'Automatic order is active. Manual ordering is disabled.'}
            </p>
          </div>
          {mode === 'manual' && (
            <button type="button" onClick={resetToAutomatic} disabled={savingMode} className="inline-flex items-center justify-center gap-2 rounded-xl border border-line bg-white px-3.5 py-2 text-xs font-bold text-slate-600 hover:bg-paper disabled:opacity-50">
              <RotateCcw size={14} /> Restore automatic
            </button>
          )}
        </div>
