...
export default function AdminSidebar({ session, tabs, activeTab, counts, roleLabels, onTabChange, onSignOut }: AdminSidebarProps) {
  return (
    <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-line bg-white lg:flex">
      ...
      <div className="border-t border-line p-3">
        <Link to="/" className="flex w-full items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold text-slate-500 transition hover:bg-paper hover:text-ink-900"><ExternalLink size={16} className="text-slate-400" />View site</Link>
        <div className="mt-2 flex items-center gap-2.5 rounded-xl bg-paper px-3.5 py-3">
          ...
          <div className="min-w-0 flex-1"><p className="truncate text-xs font-bold text-ink-900">{session.user.email}</p><p className="text-[10px] font-semibold uppercase tracking-wide text-emerald-700">{roleLabels[role] ?? role}</p></div>
