...
export default function CountryHub({ countries, brokers, contentDocs, token, notify, onNewCountry, onEditCountry }: Props) {
  ...
  const [section, setSection] = useState<'content' | 'ranking'>(() => {
    if (typeof window === 'undefined') return 'content';
    return new URLSearchParams(window.location.hash.replace(/^#/, '')).get('section') === 'ranking' ? 'ranking' : 'content';
  });
  ...
  useEffect(() => {
    let cancelled = false;
    fetch('/api/intents').then((response) => response.ok ? response.json() : []).then((data) => {
  ...
  useEffect(() => {
    const onHashChange = () => {
      ...
      setSection(params.get('section') === 'ranking' ? 'ranking' : 'content');
    };
  ...
  const switchSection = (next: 'content' | 'ranking') => {
    setSection(next);
    const params = new URLSearchParams(window.location.hash.replace(/^#/, ''));
    params.set('tab', 'countries');
    if (next === 'ranking') params.set('section', 'ranking');
