...
export default function GlobalHub({ guides, onNewGuide, onEditGuide }: GlobalHubProps) {
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-line bg-white shadow-soft">
          <div className="flex items-center justify-between border-b border-line px-5 py-4">
            <div>
              <p className="font-display text-base font-bold text-ink-900">
                Best-For pages ({bestFors.length})
              </p>
          ...
          <div className="divide-y divide-line">
            ...
            {!loading && !sortedBestFors.length && (
              <p className="p-5 text-sm text-slate-400">No canonical Best-For documents yet.</p>
            )}
          </div>
        </div>
      </div>
...
async function deleteGlobalBestFor(
  id: number,
  reload: () => Promise<void>,
...
type GlobalBestForEditorProps = {
  document: ContentDocument | null;
  onClose: () => void;
  onSaved: () => Promise<void>;
};

function GlobalBestForEditor({ document, onClose, onSaved }: GlobalBestForEditorProps) {
  const initial =
    document ??
    ({
      id: 0,
  ...
  const set = (patch: Partial<ContentDocument>) => {
    ...
    <div className="fixed inset-0 z-[100] bg-ink-950/60 p-3 backdrop-blur-sm">
      <div className="mx-auto flex h-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-white shadow-soft-lg">
        <div className="flex items-center gap-3 bg-ink-950 px-5 py-4 text-white">
          <div className="min-w-0 flex-1">
            <p className="font-display text-lg font-bold">
              {document ? 'Edit' : 'Create'} global Best-For page
            </p>
            <p className="text-xs text-slate-400">
              Canonical Content Studio document · intents remain ranking/config only
            </p>
          </div>
          <button onClick={onClose} className="rounded-lg p-2 text-slate-400">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-7">
          {error && (
            <p className="mb-4 rounded-xl bg-rose-50 px-4 py-2.5 text-sm text-rose-600">{error}</p>
          )}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label>
              ...
              <input
                ...
                placeholder="mt5-forex-brokers"
                className="h-10 w-full rounded-xl border border-line bg-paper px-3 text-sm outline-none"
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(e) => set({ content_key: e.target.value })}
                placeholder="best-for:mt5"
                className="h-10 w-full rounded-xl border border-line bg-paper px-3 text-sm outline-none"
              />
            </label>
          </div>
          ...
          <div className="mt-5">
            ...
            <PageBuilder
              ...
              onChange={(next) => {
                setBlocks(next);
                set({ blocks: next, html: blocksToHtml(next) });
              }}
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <label className="flex items-center justify-between rounded-xl border border-line bg-paper p-4">
              ...
              <button
                ...
                className={`relative h-6 w-11 rounded-full ${form.published ? 'bg-emerald-500' : 'bg-slate-300'}`}
              >
                <span
                  className={`absolute top-0.5 h-5 w-5 rounded-full bg-white ${form.published ? 'left-[22px]' : 'left-0.5'}`}
                />
              </button>
            </label>
            <label className="flex items-center justify-between rounded-xl border border-line bg-paper p-4">
              ...
              <button
                type="button"
                onClick={() => set({ indexable: !form.indexable })}
                className={`relative h-6 w-11 rounded-full ${form.indexable ? 'bg-emerald-500' : 'bg-slate-300'}`}
              >
                <span
                  className={`absolute top-0.5 h-5 w-5 rounded-full bg-white ${form.indexable ? 'left-[22px]' : 'left-0.5'}`}
                />
              </button>
            </label>
          </div>
        ...
        <div className="flex justify-end gap-2 border-t border-line bg-white px-5 py-4">
          <button onClick={onClose} className="rounded-xl border border-line px-4 py-2 text-xs font-bold text-slate-600">
            Cancel
          </button>
          <button
            onClick={save}
            disabled={busy}
            className="rounded-xl bg-ink-950 px-5 py-2 text-xs font-bold text-white disabled:opacity-60"
          >
