...
export default function CountryGuides({ country, countries, brokers, token, notify }: Props) {
  ...
  const save = async (fields: Record<string, unknown>, isNew: boolean) => {
    ...
    const payload = { ...fields, content_type: 'country-guide', country_slug: country.slug, content_key: `country-guide:${country.slug}:${slug}` };
    const res = await fetch('/api/content-documents', { method: isNew ? 'POST' : 'PUT', headers, body: JSON.stringify(isNew ? payload : { ...payload, id: editing && editing !== 'new' ? editing.id : undefined }) });
    const data = await res.json().catch(() => ({}));
