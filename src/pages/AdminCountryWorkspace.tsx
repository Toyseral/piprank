...
export default function AdminCountryWorkspace({ session, role }: { session: Session; role: string }) {
  const [countries, setCountries] = useState<CountryPage[]>([]);
  const [selected, setSelected] = useState<CountryPage | null>(null);
  ...
  const loadCountries = useCallback(async () => {
    ...
    setError('');
    try {
      const rows = await api<CountryPage[]>('/api/countries?admin=1', session.access_token);
      setCountries(Array.isArray(rows) ? rows : []);
      if (selected) setSelected(rows.find((c) => c.id === selected.id) ?? null);
    } catch (e) {
