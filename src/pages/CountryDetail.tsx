...
function BrokerRanking({ ranked, available, countrySlug }: { ranked: CountryBrokerRanking[]; available: NonNullable<CountryHubPageModel['availableBrokers']>; countrySlug: string }) {
  const [expanded, setExpanded] = useState(false);
  const rankedBrokers = ranked.filter((row): row is CountryBrokerRanking & { broker: NonNullable<CountryBrokerRanking['broker']> } => Boolean(row.broker)).slice(0, 9);
  const rankedIds = new Set(rankedBrokers.map((row) => row.broker.id));
  const remaining = available.filter((broker) => !rankedIds.has(broker.id));
  const visible = expanded
    ? [...rankedBrokers.map((row) => ({ broker: row.broker, note: row.editorial_note || undefined })), ...remaining.map((broker) => ({ broker, note: undefined }))]
    : rankedBrokers.map((row) => ({ broker: row.broker, note: row.editorial_note || undefined }));
  return <div>
    <div className="grid gap-5 md:grid-cols-2">{visible.map(({ broker, note }, index) => <BrokerCard key={broker.id} broker={broker} rank={index + 1} countrySlug={countrySlug} note={note} />)}</div>
    {!expanded && remaining.length > 0 && <button type="button" onClick={() => setExpanded(true)} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-line bg-white px-5 py-3 text-sm font-bold text-ink-900 transition hover:border-emerald-400 hover:text-emerald-700">Show {remaining.length} more brokers available in this country <ArrowRight size={15} /></button>}
    {expanded && remaining.length > 0 && <button type="button" onClick={() => setExpanded(false)} className="mt-6 text-sm font-bold text-emerald-700">Show top 9 only</button>}
  </div>;
}
...
export default function CountryDetail({ route }: { route?: CanonicalRoute }) {
  ...
  const pageSettings = (document.settings ?? {}) as Record<string, any>;
  const heroTitle = String(pageSettings.hero_title || 'Find the best forex broker in ' + (country?.name ?? '') + ' for you');
  const heroExcerpt = String(pageSettings.hero_excerpt || document.excerpt || '');
  const matcherTitle = String(pageSettings.matcher_title || 'Find a forex broker that fits your needs');
  const matcherCopy = String(pageSettings.matcher_copy || 'We already know your country. Answer a few questions about experience, trading style, platform and priorities, and the matcher will narrow the broker pool.');
  const brokerHeading = String(pageSettings.brokers_title || 'Top forex brokers available in ' + (country?.name ?? ''));
  const brokerIntro = String(pageSettings.brokers_intro || 'These brokers are currently available to traders in this country. Compare the live broker cards before you choose.');
  const mattersTitle = String(pageSettings.matters_title || 'What matters for traders in ' + (country?.name ?? ''));
  const mattersIntro = String(pageSettings.matters_intro || 'The right broker depends on more than a global score. These are the country-specific factors to check before opening an account.');
  const bestForTitle = String(pageSettings.best_for_title || 'Find a forex broker in ' + (country?.name ?? '') + ' for what matters to you');
  const bestForIntro = String(pageSettings.best_for_intro || 'Choose a trading goal to see the country-specific broker list for that use case.');
  const guidesTitle = String(pageSettings.guides_title || 'Forex broker guides for ' + (country?.name ?? ''));
  const guidesIntro = String(pageSettings.guides_intro || 'Practical guides for choosing and using a forex broker in this country.');
  const compareTitle = String(pageSettings.compare_title || 'Compare popular brokers in ' + (country?.name ?? ''));
  const compareIntro = String(pageSettings.compare_intro || 'Use head-to-head pages to inspect costs, platforms, execution, withdrawals and other documented metrics.');
  const faqTitle = String(pageSettings.faq_title || 'Forex broker questions for ' + (country?.name ?? ''));
  const finalCtaTitle = String(pageSettings.final_cta_title || 'Want a shortlist based on your needs?');
  const finalCtaCopy = String(pageSettings.final_cta_copy || 'Use the country-specific matcher above or restart it with a different profile.');
  const showSections = {
    matcher: pageSettings.show_matcher !== false,
    matters: pageSettings.show_matters !== false,
    bestFor: pageSettings.show_best_for !== false,
    guides: pageSettings.show_guides !== false,
    compare: pageSettings.show_compare !== false,
    faq: pageSettings.show_faq !== false,
    methodology: pageSettings.show_methodology !== false,
  };
    ...
    <main className="bg-paper">
      ...
      <section className="overflow-hidden border-b border-ink-900/10 bg-ink-950 text-white">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16">
          ...
          <h1 className="mt-4 max-w-4xl font-display text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">{heroTitle}</h1>
          {heroExcerpt && <p className="mt-5 max-w-3xl text-base leading-7 text-slate-300 sm:text-lg">{heroExcerpt}</p>}
      ...
      <section id="matcher" className="scroll-mt-20 border-b border-line bg-white">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:items-start lg:py-14">
          <div className="lg:sticky lg:top-24"><p className="text-[10px] font-bold uppercase tracking-[0.22em] text-emerald-700">Find your broker match</p><h2 className="mt-2 font-display text-3xl font-bold tracking-tight text-ink-950">{matcherTitle}</h2><p className="mt-4 text-sm leading-7 text-slate-600">{matcherCopy}</p><ul className="mt-5 space-y-2 text-sm text-slate-600"><li className="flex gap-2"><CheckCircle2 size={16} className="mt-0.5 text-emerald-600"/> Country-specific broker availability</li><li className="flex gap-2"><CheckCircle2 size={16} className="mt-0.5 text-emerald-600"/> Trading style and platform fit</li><li className="flex gap-2"><CheckCircle2 size={16} className="mt-0.5 text-emerald-600"/> Costs, features and personal priorities</li></ul></div>
      ...
      {ranked.length > 0 && <section id="brokers" className="scroll-mt-20 border-b border-line bg-paper"><div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16"><div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><SectionHeading eyebrow="Country ranking" title={brokerHeading} copy={brokerIntro}/><Link to="/brokers" className="inline-flex shrink-0 items-center gap-1.5 text-sm font-bold text-emerald-700">View all broker reviews <ArrowRight size={14}/></Link></div><div className="mt-8"><BrokerRanking ranked={ranked} available={brokers} countrySlug={country.slug}/></div><div className="mt-6 rounded-2xl border border-line bg-white p-5"><p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Why these brokers?</p><p className="mt-2 text-sm leading-6 text-slate-600">The ranking starts with brokers available to {country.name} residents and then applies the country × intent ranking system. Availability and ranking inputs can change, so check the current broker terms before opening an account.</p></div></div></section>}

      <section id="matters" className="scroll-mt-20 border-b border-line bg-white"><div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16"><SectionHeading eyebrow="Country-specific decision factors" title={mattersTitle} copy={mattersIntro}/><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{whatMatters.map((item: any) => <div key={item.title} className="rounded-2xl border border-line bg-paper p-5"><Globe2 size={18} className="text-emerald-700"/><h3 className="mt-3 font-display text-lg font-bold text-ink-950">{item.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{item.description}</p></div>)}</div></div></section>

      {bestFor.length > 0 && <section id="best-for" className="scroll-mt-20 border-b border-line bg-paper"><div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16"><SectionHeading eyebrow="Choose by trading goal" title={bestForTitle} copy={bestForIntro}/><div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{bestFor.slice(0,9).map((doc) => <BestForCard key={doc.content_key} doc={doc} countrySlug={country.slug}/>)}</div></div></section>}

      {guides.length > 0 && <section id="guides" className="scroll-mt-20 border-b border-line bg-paper"><div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16"><SectionHeading eyebrow="Country guides" title={guidesTitle} copy={guidesIntro}/><div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{guides.slice(0,9).map((doc) => <GuideCard key={doc.content_key} doc={doc} countrySlug={country.slug}/>)}</div><Link to={`/${country.slug}/guides`} className="mt-7 inline-flex items-center gap-2 rounded-xl bg-ink-950 px-5 py-3 text-sm font-bold text-white">Explore all {country.name} guides <ArrowRight size={15}/></Link></div></section>}
      ...
      <section id="compare" className="scroll-mt-20 border-b border-line bg-white"><div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16"><SectionHeading eyebrow="Compare brokers" title={compareTitle} copy={compareIntro}/><div className="mt-7 grid gap-3 sm:grid-cols-3">{comparisonPairs.map(({a,b}) => <Link key={a.slug+b.slug} to={`/compare/${a.slug}-vs-${b.slug}`} className="group rounded-2xl border border-line bg-paper p-5 hover:border-emerald-300 hover:bg-white"><div className="flex items-center justify-between"><span className="text-sm font-bold text-ink-950">{a.name} <span className="text-slate-400">vs</span> {b.name}</span><ArrowRight size={15} className="text-emerald-700"/></div></Link>)}</div><Link to={comparisonPath} className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-emerald-700">Open the full comparison tool <Scale size={15}/></Link></div></section>

      {faqs.length > 0 && <section id="faq" className="scroll-mt-20 border-b border-line bg-paper"><div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:py-16"><SectionHeading eyebrow="Country FAQ" title={faqTitle}/><div className="mt-8 overflow-hidden rounded-3xl border border-line bg-white">{faqs.map((faq) => <details key={faq.q} className="group border-b border-line px-5 py-5 last:border-b-0 sm:px-7"><summary className="flex cursor-pointer list-none items-center justify-between gap-6 font-display text-base font-bold text-ink-950"><span>{faq.q}</span><span className="text-xl font-normal text-slate-400 transition group-open:rotate-45">+</span></summary><p className="mt-3 pr-8 text-sm leading-7 text-slate-600">{faq.a}</p></details>)}</div></div></section>}

      <section id="methodology" className="scroll-mt-20 border-b border-line bg-white"><div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16"><div className="grid gap-6 lg:grid-cols-3"><div className="rounded-2xl border border-line bg-paper p-6"><ShieldCheck size={20} className="text-emerald-700"/><h2 className="mt-4 font-display text-xl font-bold">{String(pageSettings.methodology_title || 'How PipRank evaluates brokers')}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{String(pageSettings.methodology_copy || 'We separate country eligibility, ranking inputs and editorial controls so the local shortlist can be inspected rather than treated as a black box.')}</p></div><div className="rounded-2xl border border-line bg-paper p-6"><CheckCircle2 size={20} className="text-emerald-700"/><h2 className="mt-4 font-display text-xl font-bold">Country availability</h2><p className="mt-2 text-sm leading-6 text-slate-600">Availability, legal entities, leverage and account conditions can differ by country. Always confirm current terms with the broker.</p></div><div className="rounded-2xl border border-line bg-paper p-6"><ExternalLink size={20} className="text-emerald-700"/><h2 className="mt-4 font-display text-xl font-bold">Affiliate transparency</h2><p className="mt-2 text-sm leading-6 text-slate-600">PipRank may earn compensation when users visit or sign up with certain brokers. This does not replace the underlying ranking and editorial rules.</p></div></div><Link to={methodologyPath} className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-emerald-700">Read the full methodology <ArrowRight size={15}/></Link></div></section>

      <section className="bg-ink-950 text-white"><div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-12 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:py-16"><div><p className="text-[10px] font-bold uppercase tracking-[0.22em] text-emerald-300">Next step</p><h2 className="mt-2 font-display text-3xl font-bold sm:text-4xl">{finalCtaTitle}</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-slate-300">{finalCtaCopy}</p></div><a href="#matcher" className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-5 py-3 text-sm font-bold text-white hover:bg-emerald-400">Match Me with a Broker <ArrowRight size={16}/></a></div></section>
    </main>
  );
}
