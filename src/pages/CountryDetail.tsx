...
export default function CountryDetail() {
  ...
  const ranked = country ? country.recommended.map((r) => brokers.find((b) => b.slug === r.slug)).filter((b): b is Broker => Boolean(b)) : [];
  ...
  useSEO(seo, seo && country ? [
    ...
    buildItemListJsonLd(`Recommended forex brokers in ${country.name}`, ranked.map((b) => ({ name: b.name, path: `/brokers/${b.slug}` }))),
  ] : undefined);
