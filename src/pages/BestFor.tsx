...
export default function BestFor() {
  ...
  const ranked = useMemo(() => {
    ...
    const intentPool = countrySlug && countryRankingIds.length ? pool.filter((b) => countryRankingIds.includes(b.id)) : pool.filter((b) => b.best_for.includes(intent.slug));
  ...
  const rest = useMemo(() => !intent || countrySlug ? [] : brokers.filter((b) => !b.best_for.includes(intent.slug)).slice(0, 4), [brokers, intent, countrySlug]);
  const seoInput: SeoInput | null = intent ? (countrySlug ? countryBestForSeo(countrySlug, intent) : intentSeo(intent)) : null;
  const seoJsonLd = intent && seoInput ? [buildBreadcrumbJsonLd(countrySlug ? [{ name: 'Home', path: '/' }, { name: 'Countries', path: '/countries' }, { name: country?.name ?? countrySlug, path: `/countries/${countrySlug}` }, { name: intent.title, path: seoInput.path }] : [{ name: 'Home', path: '/' }, { name: 'Best Forex Brokers', path: bestForPath(intent.slug) }, { name: intent.title, path: seoInput.path }]), buildItemListJsonLd(intent.title, ranked.slice(0, 10).map((b) => ({ name: b.name, path: `/brokers/${b.slug}` }))), ...('faqs' in intent && intent.faqs?.length ? [buildFAQPageJsonLd(intent.faqs.map((f) => ({ question: f.q, answer: f.a })))] : [])] : undefined;
