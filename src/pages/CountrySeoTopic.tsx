...
export default function CountrySeoTopic() {
    ...
    ? {
        ...
        alternates: [
          ...
          ...localizedAlts
            ...
            .map((r) => ({ hreflang: r.locale || r.language_code || 'und', path: `/${country.slug}/${r.url_prefix}/${r.slug}` })),
        ],
      }
    : null;
  ...
  useSEO(
    ...
    seo && country && topic
      ? [
          ...
          buildItemListJsonLd(`${topic.title} in ${country.name}`, ranked.slice(0, 10).map((b) => ({ name: b.name, path: `/brokers/${b.slug}` }))),
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
      ...
      {richContent?.published && (Array.isArray(richContent.blocks) && richContent.blocks.length ? <section className="mt-10" aria-label="Additional editorial content"><PageBlocksRenderer blocks={richContent.blocks as any} brokers={brokers} intent={topic.slug} countrySlug={country.slug} className="piprank-rich-content space-y-8" /></section> : richContent.html ? <section className="mt-10 rounded-2xl border border-line bg-white p-6 sm:p-8" aria-label="Additional editorial content">{richContent.title && <h2 className="font-display text-2xl font-bold text-ink-950">{richContent.title}</h2>}{richContent.excerpt && <p className="mt-2 text-sm leading-6 text-slate-500">{richContent.excerpt}</p>}<div className="piprank-rich-content mt-6" dangerouslySetInnerHTML={{ __html: richContent.html }} /></section> : null)}
