...
import { ArrowRight, BadgeCheck, Building2, CircleDollarSign, Globe2, Monitor, ShieldCheck } from 'lucide-react';
...
function FixedDataCard({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
...
export default function BrokerDetailRedesign() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchBroker(slug),
      fetchReviewsForSlug(slug),
    ]).then(async ([b, r]) => {
      if (!active) return;
      setBroker(b);
      setReviews(r);
      const [bc, doc] = await Promise.all([
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl bg-white p-2">{broker.logo_url ? <img src={broker.logo_url} alt={`${broker.name} logo`} className="h-full w-full object-contain" /> : <span className="font-display text-xl font-bold text-ink-950">{broker.name.slice(0, 2).toUpperCase()}</span>}</div>
                <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-300">Broker review</p><h1 className="mt-1 font-display text-4xl font-bold tracking-tight sm:text-5xl">{broker.name}</h1></div>
              </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 lg:min-w-[260px]">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
          <div className="min-w-0 space-y-8">
            <section aria-labelledby="overview" className="rounded-3xl border border-line bg-white p-5 shadow-soft sm:p-7">
              <div className="flex items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">Overview</p><h2 id="overview" className="mt-1 font-display text-2xl font-bold text-ink-950">{broker.name} at a glance</h2></div><Globe2 className="text-slate-300" /></div>
              <div className="mt-6"><SystemFacts broker={broker} /></div>
              <div className="mt-6"><TextList items={content?.overview ?? broker.review} /></div>
            </section>

            <section><PipRankVerdictCard broker={broker} text={content?.verdict?.join(' ')} /></section>

            <section><PageBlocksRenderer blocks={blocks.filter((b: any) => !b.zone || b.zone === 'editorial')} brokers={[broker]} className="space-y-6" /></section>

            <FixedDataCard title="Pricing & trading costs" icon={<CircleDollarSign size={19} />}>
              <div className="grid gap-3 sm:grid-cols-2"><div><span className="text-xs text-slate-500">Minimum deposit</span><strong className="mt-1 block text-lg">{fmtMoney(broker.min_deposit)}</strong></div><div><span className="text-xs text-slate-500">EUR/USD spread</span><strong className="mt-1 block text-lg">{broker.spread_eurusd} pips</strong></div><div><span className="text-xs text-slate-500">Commission</span><strong className="mt-1 block text-lg">{broker.commission || '—'}</strong></div><div><span className="text-xs text-slate-500">Inactivity fee</span><strong className="mt-1 block text-lg">{broker.inactivity_fee || '—'}</strong></div></div>
              <div className="mt-5"><TextList items={content?.fees_detail} /></div>
              <div className="mt-5"><PageBlocksRenderer blocks={blocks.filter((b: any) => b.zone === 'pricing-content')} brokers={[broker]} className="space-y-5" /></div>
            </FixedDataCard>

            <FixedDataCard title="Trading platforms" icon={<Monitor size={19} />}>
              <div className="flex flex-wrap gap-2">{broker.platforms.map((platform) => <span key={platform} className="rounded-full border border-line bg-paper px-3 py-2 text-sm font-bold">{platform}</span>)}</div>
              <div className="mt-5"><TextList items={content?.platform_intro} /></div>
              <div className="mt-5"><PageBlocksRenderer blocks={blocks.filter((b: any) => b.zone === 'platform-content')} brokers={[broker]} className="space-y-5" /></div>
            </FixedDataCard>

            <FixedDataCard title="Trust & regulation" icon={<ShieldCheck size={19} />}>
              <div className="grid gap-3 sm:grid-cols-2">{broker.regulations.map((r) => <div key={`${r.body}-${r.country}`} className="rounded-2xl bg-paper p-4"><p className="font-bold text-ink-950">{r.body}</p><p className="mt-1 text-xs text-slate-500">{r.country} · Tier {r.tier}</p></div>)}</div>
              <div className="mt-5"><TextList items={content?.regulation_detail} /></div>
              <div className="mt-5"><PageBlocksRenderer blocks={blocks.filter((b: any) => b.zone === 'trust-content')} brokers={[broker]} className="space-y-5" /></div>
            </FixedDataCard>

            <FixedDataCard title="Account types & funding" icon={<Building2 size={19} />}>
              <div className="grid gap-3 sm:grid-cols-2">{broker.account_types.map((account) => <div key={account} className="rounded-2xl border border-line p-4 text-sm font-semibold">{account}</div>)}</div>
              <div className="mt-5"><TextList items={content?.accounts_intro} /></div>
              {content?.funding_intro?.length ? <div className="mt-5"><TextList items={content.funding_intro} /></div> : null}
            </FixedDataCard>

            <section><PipRankVerdictCard broker={broker} headline={`Why consider ${broker.name}?`} text={content?.why_recommend?.join(' ')} showCta={false} /></section>
...
async function fetchReviewsForSlug(slug: string): Promise<Review[]> {
  const broker = await fetchBroker(slug);
  return fetchReviews(broker.id);
}
