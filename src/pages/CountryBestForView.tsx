...
import { fetchBrokers, fetchCountry, fetchCountryBestFor, fetchCountryIntentRankings } from '../lib/api';
...
export default function CountryBestForView({ countrySlug, slug, contentKey, document: initialDocument }: { countrySlug: string; slug: string; contentKey?: string; document?: ContentDocument | null }) {
  ...
  const [page, setPage] = useState<CountryBestFor | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (active) { setCountry(null); setPage(null); setDoc(null); }
  ...
  if (!country || !page || page.indexable === false || (doc && !doc.published)) return <NotFound />;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      ...
      {ranked.length > 0 && <section className="mt-8"><p className="text-xs font-bold uppercase tracking-widest text-emerald-700">PipRank recommendations</p><h2 className="mt-1 font-display text-2xl font-bold">Best {page.label || page.title} in {country.name}</h2><div className="mt-5 grid gap-5 lg:grid-cols-2">{ranked.map((b, i) => <Reveal key={b.slug} delay={Math.min(i, 5) * 0.05}><BrokerCard broker={b} rank={i + 1} intent={canonicalIntent} countrySlug={country.slug} /></Reveal>)}</div></section>}
