      ...
      <section className="border-b border-line bg-ink-950 text-white">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center lg:gap-10">
            <div className="flex min-w-0 items-start gap-4 sm:gap-5"><Monogram name={broker.name} logoUrl={broker.logo_url} color={broker.brand_color} size={76} className="shrink-0 rounded-2xl ring-2 ring-white/15" /><div className="min-w-0"><h1 className="font-display text-3xl font-bold tracking-tight sm:text-5xl">{broker.name}</h1><p className="mt-2 max-w-3xl text-sm leading-7 text-slate-300 sm:text-base">{broker.tagline}</p><div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-300"><BadgeCheck size={14} className="text-emerald-300" /><span>Regulation</span><span className="text-slate-500">·</span><span>{broker.regulations.length ? broker.regulations.slice(0, 2).map((r) => r.body).join(' · ') : 'Regulatory information available in this review'}</span></div></div></div>
