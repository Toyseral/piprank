...
function Anchors({ mobile = false }: { mobile?: boolean }) {
  return (
    <nav aria-label="On this page" className={mobile ? 'overflow-x-auto border-y border-line bg-paper py-3' : 'rounded-3xl border border-line bg-white p-5 shadow-soft'}>
      <div className={mobile ? 'flex min-w-max gap-2' : 'space-y-1'}>
        {anchors.map(([id, label]) => (
          <a key={id} href={`#${id}`} className={mobile ? 'rounded-full border border-line bg-white px-3 py-1.5 text-xs font-bold text-slate-600' : 'block rounded-xl px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-paper hover:text-emerald-700'}>{label}</a>
        ))}
      </div>
    </nav>
  );
          ...
          <div className="mt-7 grid gap-8 lg:grid-cols-[1fr_280px] lg:items-end">
            <div>
              <div className="flex items-center gap-4">
                ...
                <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-300">Broker review</p><h1 className="font-display text-4xl font-bold sm:text-5xl">{broker.name}</h1></div>
              </div>
              ...
              <div className="mt-6 grid max-w-3xl grid-cols-2 gap-2 sm:grid-cols-4">{[['Platform', broker.platforms.slice(0, 2).join(' · ') || '—'], ['Min deposit', fmtMoney(broker.min_deposit)], ['Trust score', `${broker.trust_score}/100`], ['EUR/USD spread', `${broker.spread_eurusd} pips`]].map(([label, value]) => <div key={label} className="rounded-2xl border border-white/10 bg-white/[0.07] px-3 py-3"><p className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">{label}</p><p className="mt-1 text-sm font-bold text-white">{value}</p></div>)}</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-5"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">PipRank score</p><p className="mt-1 font-display text-4xl font-bold text-emerald-300">{score}<span className="text-sm text-slate-400">/100</span></p><div className="mt-4"><VisitButton broker={broker} className="w-full justify-center" /></div></div>
          </div>
        </div>
      </section>
        ...
        <div className="mt-7 grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
          <div className="min-w-0 space-y-8">
            {hasCanonicalDocument ? <>
              <section id="overview" className="scroll-mt-28 rounded-3xl border border-line bg-white p-5 shadow-soft sm:p-7"><div className="border-b border-line pb-5"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-700">Overview</p><h2 className="mt-1 font-display text-2xl font-bold text-ink-950">{broker.name} at a glance</h2></div><div className="pt-6"><Copy items={content?.overview ?? broker.review} /><div className="mt-6"><StructuredBrokerDataCard broker={broker} section="overview" /></div></div></section>
              ...
              <Shell id="overview" eyebrow="Overview" title={`${broker.name} at a glance`}><Copy items={content?.overview ?? broker.review} /><div className="mt-6"><StructuredBrokerDataCard broker={broker} section="overview" /></div></Shell>
              <section id="verdict" className="scroll-mt-28"><PipRankVerdictCard broker={broker} text={content?.verdict?.join(' ')} /></section>
              <section id="what-we-like" className="scroll-mt-28"><StructuredBrokerDataCard broker={broker} section="editorial" /></section>
              ...
              <Shell id="accounts" eyebrow="Account types" title="Choose the account that fits"><div className="overflow-hidden rounded-2xl border border-line bg-paper px-4 sm:px-5">{broker.account_types.map((account) => <div key={account} className="flex items-center justify-between gap-5 border-b border-line/70 py-4 last:border-b-0"><div><p className="font-display font-bold text-ink-950">{account}</p><p className="mt-1 text-xs text-slate-500">Minimum deposit {fmtMoney(broker.min_deposit)}</p></div><p className="text-right text-sm font-bold text-ink-950">{broker.spread_eurusd} pips<br /><span className="text-xs font-normal text-slate-500">EUR/USD spread</span></p></div>)}</div><Copy items={content?.accounts_intro} /></Shell>
