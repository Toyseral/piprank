...
import { allInCost, healthScore } from '../lib/score';
  ...
  return <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
    <div className="relative overflow-hidden rounded-3xl bg-ink-950 p-7 sm:p-10">
      ...
      <div className="relative">
        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-500 text-ink-950 shadow-lg shadow-emerald-500/30"><Icon size={24} /></div>
        <p className="mt-4 text-xs font-bold uppercase tracking-widest text-emerald-300">PipRank</p>
        <h1 className="mt-3 font-display text-3xl font-bold tracking-tight text-white sm:text-4xl">{document.title}</h1>
        {document.excerpt && <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-400 sm:text-[15px]">{document.excerpt}</p>}
      </div>
    </div>

    <section className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50/50 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><p className="text-xs font-bold uppercase tracking-widest text-emerald-700">Your next step</p><h2 className="mt-1 font-display text-lg font-bold text-ink-900">Choose the broker that fits you best</h2></div>
        {ranked[0] && <Link to={`/brokers/${ranked[0].slug}`} className="inline-flex items-center gap-2 rounded-xl bg-ink-950 px-4 py-2.5 text-sm font-bold text-white hover:bg-ink-800">Read the top pick <ArrowRight size={15} /></Link>}
      </div>
    </section>

    <div className="mt-6 rounded-2xl border border-line bg-white p-6">
      <p className="text-xs font-bold uppercase tracking-widest text-slate-400">How we ranked this list</p>
      <ul className="mt-3 grid gap-2.5 sm:grid-cols-2">{intent.criteria.map((criterion) => <li key={criterion} className="flex gap-2.5 text-sm text-slate-600"><Check size={16} className="mt-0.5 shrink-0 text-emerald-600" strokeWidth={3} />{criterion}</li>)}</ul>
    </div>

    <div className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-2">
      {ranked.map((broker, index) => <Reveal key={broker.slug} delay={Math.min(index, 5) * 0.05}><BrokerCard broker={broker} rank={index + 1} note={reasonFor(intent.slug, broker)} intent={intent.slug} /></Reveal>)}
    </div>
...
function documentTitle(title: string) {
  if (typeof document !== 'undefined') document.title = `${title} | PipRank`;
}
