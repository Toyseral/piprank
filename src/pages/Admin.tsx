...
import PageBuilder, { blocksToHtml, hasVisualContent, type PageBlock } from '../components/PageBuilder';
...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, s) => {
      // Supabase emits TOKEN_REFRESHED after a background tab is restored.
      // Replacing the Session here makes the role effect remount Dashboard and
      // discards every unsaved visual-builder edit. Keep the mounted workspace
      // for token refreshes; the current token is refreshed by Supabase itself.
      const nextUserId = s?.user.id ?? null;
      if (event === 'TOKEN_REFRESHED' && nextUserId === sessionUserId.current) return;
              ...
              <>
                ...
                {activeTab === 'global' && (
                  ...
                  <PageManager
                    ...
                    contentDocs={contentDocs}
                    token={session.access_token}
                    onSave={(fields, isNew) => mutate('/api/content-documents', isNew ? 'POST' : 'PUT', fields, isNew ? 'Content page created' : 'Content page saved')}
                    onDelete={(doc) => { if (window.confirm(`Delete “${doc.title || doc.content_key}”?`)) mutate('/api/content-documents', 'DELETE', { id: doc.id }, 'Content page deleted'); }}
                  />
                  </div>
                )}
                {activeTab === 'authors' && (
...
function BrokerRichDocEditor({ broker, token, document, seedBlocks, seedTitle, onClose, onSave }: { broker: Broker; token: string; document: ContentDocument | null; seedBlocks?: PageBlock[]; seedTitle?: string; onClose:()=>void; onSave:(doc:any,isNew:boolean)=>Promise<void> }) {
  ...
  const [builderBlocks,setBuilderBlocks]=useState<any>(()=>hasVisualContent(form.blocks) ? form.blocks : (form.html ? { html: form.html } : []));
...
function ContentDocumentEditor({ document, countries, token, defaultCountrySlug, onClose, onSave }: {
  ...
  return <DrawerShell title={document ? 'Edit rich content' : 'New rich content'} onClose={onClose} wide>
    <div className="space-y-5">
      ...
      <div><FieldLabel hint="Build the complete country SEO editorial page visually">Visual page builder</FieldLabel><div className="mt-1.5"><PageBuilder value={hasVisualContent(form.blocks) ? form.blocks : (form.html ? { html: form.html } : [])} pageType={form.content_type === 'broker' ? 'broker' : form.content_type === 'guide' ? 'guide' : form.content_type === 'country' ? 'country' : 'standalone'} onChange={document=>setForm({...form,blocks:document as any,html:blocksToHtml(document)})} onUploadImage={async (file) => { const reader = new FileReader(); const data = await new Promise<string>((resolve, reject) => { reader.onload=()=>resolve(String(reader.result)); reader.onerror=reject; reader.readAsDataURL(file); }); const res = await fetch('/api/content-assets', { method:'POST', headers:{'Content-Type':'application/json', Authorization:`Bearer ${token}`}, body:JSON.stringify({ filename:file.name, contentType:file.type, dataBase64:data }) }); const out=await res.json().catch(()=>({})); if(!res.ok) throw new Error(out.error || 'Image upload failed'); return out.url; }} /></div></div>
