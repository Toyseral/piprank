// Remove two-segment country prerenders that are not owned by a published
// country Best-For document. Country guides live under /:country/guides/:slug.
...
async function main() {
  ...
  if (!url || !key) {
    if (process.env.VERCEL_ENV === 'production') throw new Error('Supabase credentials are required to clean non-canonical country prerenders.');
    console.log('[remove-noncanonical-country-prerenders] Non-production build without Supabase credentials — skipped.');
    return;
  }
  ...
  console.log(`[remove-noncanonical-country-prerenders] Removed ${removed} non-canonical two-segment country prerenders.`);
}

main().catch((error) => {
  console.error('[remove-noncanonical-country-prerenders] ERROR:', error);
  process.exit(1);
});
