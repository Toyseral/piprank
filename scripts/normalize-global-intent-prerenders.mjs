...
const MAP = {
  beginners: 'forex-brokers-for-beginners', 'low-spread': 'low-spread-forex-brokers', mt5: 'mt5-forex-brokers', mt4: 'mt4-forex-brokers',
  gold: 'gold-forex-brokers', ecn: 'ecn-forex-brokers', 'copy-trading': 'copy-trading-forex-brokers',
  scalping: 'forex-brokers-for-scalping', 'swing-trading': 'forex-brokers-for-swing-trading', 'high-leverage': 'high-leverage-forex-brokers',
};
...
for (const [legacySlug, canonicalSlug] of Object.entries(MAP)) {
  ...
  if (existsSync(sourceHtml)) {
    ...
    html = html.split(`/best/${legacySlug}`).join(`/${canonicalSlug}`);
...
if (existsSync(bestDir)) rmSync(bestDir, { recursive: true, force: true });
