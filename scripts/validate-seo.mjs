...
function htmlFiles(dir) {
  ...
  const out = [];
  for (const entry of readdirSync(dir, { withFileTypes: true })) { const full = join(dir, entry.name); if (entry.isDirectory()) out.push(...htmlFiles(full)); else if (entry.name === 'index.html') out.push(full); }
  return out;
}
...
for (const file of files) {
  ...
  if (canonicals.length !== 1) errors.push(`${label}: expected exactly one canonical, found ${canonicals.length}`);
    ...
    const expectedCanonical = siteUrl + label;
    if (canonicals[0] !== expectedCanonical) errors.push(`${label}: canonical mismatch; expected ${expectedCanonical}, found ${canonicals[0]}`);
  }
  if (!ogTitle || ogTitle !== title) errors.push(`${label}: og:title does not match <title>`);
  if (!ogDescription || ogDescription !== description) errors.push(`${label}: og:description does not match meta description`);
  if (h1s !== 1) errors.push(`${label}: expected exactly one H1, found ${h1s}`);
  if (body.replace(/\s+/g, '').length < 250) errors.push(`${label}: prerendered body is suspiciously thin`);
  if (jsonLdCount < 1) warnings.push(`${label}: no JSON-LD block found`);
...
if (!existsSync(sitemapFile)) errors.push('sitemap.xml is missing.');
  ...
  const xml = readFileSync(sitemapFile, 'utf8'); const locs = [...xml.matchAll(/<loc>(.*?)<\/loc>/g)].map((m) => m[1]);
  ...
  for (const loc of locs) {
    ...
    if (seenLocs.has(loc)) errors.push(`sitemap contains duplicate URL: ${loc}`); seenLocs.add(loc);
    try {
      const path = new URL(loc).pathname; const relativePath = decodePath(path).replace(/^\//, ''); const file = join(DIST, relativePath, 'index.html');
      if (relativePath === '') { if (!existsSync(join(DIST, 'index.html'))) errors.push(`sitemap URL has no generated HTML: ${loc}`); }
      else if (!existsSync(file)) errors.push(`sitemap URL has no generated HTML: ${loc}`);
    } catch { errors.push(`sitemap contains invalid URL: ${loc}`); }
  }
}

const robotsFile = join(DIST, 'robots.txt');
if (!existsSync(robotsFile)) errors.push('robots.txt is missing.');
else { const robots = readFileSync(robotsFile, 'utf8'); if (!robots.includes(`Sitemap: ${siteUrl}/sitemap.xml`)) errors.push('robots.txt does not reference the configured sitemap URL.'); }
