...
const replacement = [
  "  const shortList = (values, max = 3) => {",
  "    if (!values || !values.length) return 'Not listed';",
  ...
  "  const brokerScore = (b) => Math.max(0, Math.min(100, Math.round(Number(b.trust_score ?? (Number(b.rating || 0) * 20) ?? 0))));",
  ...
  "      '<div class=\"flex items-start gap-3\">' + logo + '<div class=\"min-w-0 flex-1\"><h3 class=\"truncate font-display text-base font-bold text-ink-900\">' + esc(b.name) + '</h3><p class=\"truncate text-xs text-slate-500\">' + esc(b.tagline || '') + '</p><div class=\"mt-1.5 flex items-center gap-1.5\"><span class=\"text-amber-500\">★★★★★</span><span class=\"tnum text-xs font-bold text-ink-900\">' + Number(b.rating || 0).toFixed(1) + '</span></div></div></div>' +",
  ...
  "        '<div class=\"min-w-0 rounded-xl bg-paper px-3 py-2\"><p class=\"text-[10px] font-semibold uppercase tracking-wide text-slate-400\">Max leverage</p><p class=\"tnum mt-0.5 truncate font-bold leading-tight text-ink-900 text-[12px] sm:text-sm\">' + esc(b.max_leverage ?? '—') + '</p></div></div>' +",
  ...
  "      const rows = section === 'pricing' ? [['Minimum deposit', money(broker.min_deposit)], ['EUR/USD spread', (broker.spread_eurusd ?? '—') + ' pips'], ['Commission', broker.commission || '—'], ['Max leverage', broker.max_leverage || '—']] : section === 'platforms' ? [['Platforms', (broker.platforms || []).join(', ') || '—'], ['Account types', (broker.account_types || []).join(', ') || '—']] : [['Rating', Number(broker.rating || 0).toFixed(1) + '/5'], ['Trust score', (broker.trust_score ?? '—') + '/100'], ['Regulation', shortList((broker.regulations || []).map((r) => r?.body).filter(Boolean))], ['Founded', String(broker.founded ?? '—')]];",
