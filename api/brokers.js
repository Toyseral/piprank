...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (requestedSlug) { const { data, error } = await supabase.from('brokers').select(PUBLIC_BROKER_FIELDS).eq('slug', requestedSlug).maybeSingle(); if (error) throw error; if (!data) return res.status(404).json({ error: 'Broker not found' }); return res.status(200).json(data); }
      const { data, error } = await supabase.from('brokers').select(PUBLIC_BROKER_FIELDS).order('rating', { ascending: false }); if (error) throw error; return res.status(200).json(data ?? []);
    }
    if (!(await requireRole(req, res, BROKER_WRITE))) return;
    if (req.method === 'POST') { const body = req.body ?? {}; if (!body.name || String(body.name).trim().length < 2) return res.status(400).json({ error: 'Broker name is required' }); const payload = { ...BROKER_DEFAULTS, ...pickBrokerFields(body), name: String(body.name).trim(), slug: body.slug ? slugify(body.slug) : slugify(body.name) }; const { data, error } = await supabase.from('brokers').insert(payload).select().single(); if (error) throw error; return res.status(201).json(data); }
