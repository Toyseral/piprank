...
async function updateTable(table, id, blocks) {
  const { data, error } = await supabase.from(table).update({ blocks: cleanBlocks(blocks) }).eq('id', Number(id)).select('*').single();
...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    const table = type === 'guide' ? 'guides' : type === 'intent' ? 'intents' : type === 'country_best_for' ? 'country_best_for' : null;
  ...
  const { type, id, blocks } = req.body ?? {};
  const table = type === 'guide' ? 'guides' : type === 'intent' ? 'intents' : type === 'country_best_for' ? 'country_best_for' : null;
  if (!table || !id) return res.status(400).json({ error: 'type, id and blocks are required' });
  return res.status(200).json(await updateTable(table, id, blocks));
}
