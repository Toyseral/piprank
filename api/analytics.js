...
async function clicks(req, res) {
  if (req.method === 'POST') {
    ...
    const { data: broker } = await supabase.from('brokers').select('id').eq('id', Number(broker_id)).maybeSingle(); if (!broker) return res.status(404).json({ error: 'Broker not found' });
    const { error } = await supabase.from('clicks').insert({ broker_id: Number(broker_id), page: pageValue }); if (error) throw error;
...
async function affiliateLinks(req, res) {
  ...
  if (req.method === 'GET') { const brokerId = req.query?.broker_id ? Number(req.query.broker_id) : null; let query = supabase.from('affiliate_links').select('*').order('broker_id').order('country_code', { nullsFirst: true }); if (brokerId) query = query.eq('broker_id', brokerId); const { data, error } = await query; if (error) throw error; return res.status(200).json(data ?? []); }
  if (req.method === 'POST') {
    const body = req.body ?? {}; if (!body.broker_id) return res.status(400).json({ error: 'broker_id is required' });
    if (!body.affiliate_url || !/^https:\/\//i.test(String(body.affiliate_url))) return res.status(400).json({ error: 'A valid HTTPS affiliate_url is required' });
  ...
  if (req.method === 'PUT') {
    ...
    if ('affiliate_url' in fields && !/^https:\/\//i.test(String(fields.affiliate_url))) return res.status(400).json({ error: 'A valid HTTPS affiliate_url is required' }); if ('direct_url' in fields && fields.direct_url && !/^https:\/\//i.test(String(fields.direct_url))) return res.status(400).json({ error: 'direct_url must be HTTPS' });
