...
function rateLimited(req) {
  ...
  if (rateBuckets.size > 5000) {
    for (const [k, v] of rateBuckets) if (now - v.started > WINDOW_MS) rateBuckets.delete(k);
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!Number.isInteger(brokerId) || brokerId <= 0 || !cleanAuthor || !cleanTitle || !cleanBody)
      ...
      const fingerprint = reviewFingerprint({ brokerId, author: cleanAuthor.toLowerCase(), title: cleanTitle.toLowerCase(), body: cleanBody.toLowerCase() });
      const { data: recent, error: duplicateError } = await supabase
        .from('reviews')
        .select('id')
        .eq('broker_id', brokerId)
        .eq('author', cleanAuthor.slice(0, 40))
        .eq('title', cleanTitle.slice(0, 90))
      ...
      const { data, error } = await supabase.from('reviews').insert({
        ...
        verified,
        helpful: 0,
        // Kept in request scope for deterministic abuse logging without storing IPs.
        metadata: { fingerprint },
      }).select().single();
      if (error) {
        // Older deployments may not yet have the optional metadata column.
        if (error.code === 'PGRST204' || /metadata/i.test(String(error.message ?? ''))) {
          const fallback = await supabase.from('reviews').insert({ broker_id: brokerId, author: cleanAuthor, country: String(country || 'Not specified').trim().slice(0, 40), rating: r, title: cleanTitle, body: cleanBody, verified, helpful: 0 }).select().single();
    ...
    if (req.method === 'PUT') {
      ...
      const { data, error } = await supabase.rpc('increment_review_helpful', { review_id: Number(id) });
      if (error) {
        if (error.code === 'PGRST202' || /increment_review_helpful/i.test(String(error.message ?? ''))) {
