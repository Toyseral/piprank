...
async function handleContent(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    const payload = {
      ...
      faqs: Array.isArray(body.faqs) ? body.faqs : [], platforms: Array.isArray(body.platforms) ? body.platforms : [], accounts: Array.isArray(body.accounts) ? body.accounts : [],
      payments: Array.isArray(body.payments) ? body.payments : [], blocks: Array.isArray(body.blocks) ? body.blocks : [], updated_at: new Date().toISOString(),
    };
...
async function handleAvailability(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    const rows=Array.isArray(body.rows)?body.rows:[]; const {error:delError}=await supabase.from('broker_country_availability').delete().eq('broker_id',broker_id); if(delError)throw delError;
...
export default async function handler(req,res){try{const resource=String(req.query.resource||'content');if(resource==='content')return handleContent(req,res);if(resource==='availability')return handleAvailability(req,res);if(resource==='verification')return handleVerification(req,res);if(resource==='media')return handleMedia(req,res);return res.status(404).json({error:'Unknown resource'});}catch(error){console.error(error);return res.status(500).json({error:error instanceof Error?error.message:'Internal server error'});}}
