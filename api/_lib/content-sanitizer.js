...
const BLOCK_KEYS = {
  richtext: ['id','type','title','html','editorialSection'],
  heading: ['id','type','title','editorialSection'],
  image: ['id','type','title','src','alt','editorialSection'],
  table: ['id','type','title','rows','editorialSection'],
  callout: ['id','type','title','html','tone','editorialSection'],
  divider: ['id','type','title','editorialSection'],
  links: ['id','type','title','links','editorialSection'],
  structured_broker_data: ['id','type','title','brokerId','section','editorialSection'],
  broker_card: ['id','type','title','brokerId','variant','editorialSection'],
  broker_grid: ['id','type','title','brokerIds','variant','editorialSection'],
  comparison_table: ['id','type','title','brokerIds','fields','ctaLabel','showCta','editorialSection'],
...
function cleanStringArray(value, max = 100, itemMax = 300) {
  return Array.isArray(value)
    ? value.filter((item) => typeof item === 'string').map((item) => cleanText(item, itemMax)).filter(Boolean).slice(0, max)
    : [];
}

function cleanRows(value) {
  return Array.isArray(value)
    ? value.slice(0, 100).map((row) => Array.isArray(row) ? row.slice(0, 20).map((cell) => cleanText(cell, 500)) : []).filter((row) => row.length)
    : [];
}

function cleanLinks(value) {
  return Array.isArray(value)
    ? value.slice(0, 100).map((link) => ({ label: cleanText(link?.label, 180), href: safeUrl(link?.href) || '#' })).filter((link) => link.label)
    : [];
}
...
export function sanitizeBlock(block) {
  ...
  if ('html' in output) output.html = sanitizeHtml(output.html);
...
export function sanitizeBlocks(blocks) {
  return Array.isArray(blocks) ? blocks.map(sanitizeBlock).filter(Boolean).slice(0, 200) : [];
}

export function sanitizePublicSettings(settings) {
  if (!settings || typeof settings !== 'object' || Array.isArray(settings)) return {};
  const output = {};
  const keys = ['locale','languageCode','icon','label','criteria','sections','faqs','ranking_intent_slug','canonicalIntentSlug','image','rankingMode','pinnedBrokerSlugs','excludedBrokerSlugs','comparisonFields'];
